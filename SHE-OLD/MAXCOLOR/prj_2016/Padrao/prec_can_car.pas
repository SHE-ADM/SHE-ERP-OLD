...
type
  Tfrmrec_can_car = class(Tfrmpadr3)
...
var
  frmrec_can_car: Tfrmrec_can_car;
  Ano, Mes, Dia: Word;
  demi: TDate;
  chav,cdnf: String;
...
procedure Tfrmrec_can_car.ALTERA_STFI(tipo,doct: string);
begin
  with consulta_S do
  begin
    SQL.Clear;
...
procedure Tfrmrec_can_car.INCLUI_CONTA;
begin
  try
    tag := 1;
...
procedure Tfrmrec_can_car.PESQUISA_CONTA(BRet: boolean;txt: string);
begin
  with fin_rec_bai do
  begin
    SQL.Clear;
...
procedure Tfrmrec_can_car.AjustaForm;
begin
...
procedure Tfrmrec_can_car.ABRE_TABELA;
begin
  oOTransact(TSheild);
  with consulta_s do
  begin
...
procedure Tfrmrec_can_car.siPSQClick(Sender: TObject);
begin
  if cadastro.State in [dsEdit,dsInsert] then
     cadastro.Post;
...
procedure Tfrmrec_can_car.FormCreate(Sender: TObject);
begin
  inherited;
...
procedure Tfrmrec_can_car.cadastroAfterCancel(DataSet: TDataSet);
begin
  if tag = 0 then
  begin
    finA.Enabled := true;
...
procedure Tfrmrec_can_car.cadastroAfterDelete(DataSet: TDataSet);
begin
  tSHEILD.CommitRetaining;
end;

procedure Tfrmrec_can_car.cadastroAfterEdit(DataSet: TDataSet);
begin
  if tag = 0 then
  begin
    finA.Enabled := false;
...
procedure Tfrmrec_can_car.cadastroAfterOpen(DataSet: TDataSet);
begin
  if tag = 0 then
  begin
    finA.Enabled := true;
...
procedure Tfrmrec_can_car.cadastroBeforeCancel(DataSet: TDataSet);
begin
  if tag = 0 then
  begin
    finA.Enabled := true;
...
procedure Tfrmrec_can_car.cadastroAfterPost(DataSet: TDataSet);
var
  wRec: TBookMark;
begin
  tSHEILD.CommitRetaining;
...
procedure Tfrmrec_can_car.finAClick(Sender: TObject);
begin
  cadastro.Edit;
end;

procedure Tfrmrec_can_car.finDClick(Sender: TObject);
begin
  if not cadastro.Fields[0].IsNull then
  cadastro.Delete;
end;

procedure Tfrmrec_can_car.siSAVClick(Sender: TObject);
begin
  cadastro.Post;
end;

procedure Tfrmrec_can_car.finCClick(Sender: TObject);
begin
  cadastro.Cancel;
end;

procedure Tfrmrec_can_car.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  case key of
...
procedure Tfrmrec_can_car.dbgConsultaKeyDown(Sender: TObject;
...
procedure Tfrmrec_can_car.siVALClick(Sender: TObject);
begin
  if cadastro.State in [dsEdit,dsInsert] then
  cadastro.Post;

  if oYesNo(handle,'Validar cancelamento em carteira ?') = mrno then
  abort;
...
procedure Tfrmrec_can_car.dbgConsultaCustomDrawCell(Sender: TObject;
...
procedure Tfrmrec_can_car.cadastroBeforePost(DataSet: TDataSet);
begin
...
procedure Tfrmrec_can_car.cadastroFIN_STFIValidate(Sender: TField);
begin
  if tag = 0 then
  begin
...
procedure Tfrmrec_can_car.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  inherited;
  if frmrec_can_car <> nil then
  begin
    frmrec_can_car.Release;
    frmrec_can_car := nil;
  end;
end;

procedure Tfrmrec_can_car.FormCloseQuery(Sender: TObject;
...
procedure Tfrmrec_can_car.cadastroBeforeDelete(DataSet: TDataSet);
begin
...
procedure Tfrmrec_can_car.lan_chqAfterPost(DataSet: TDataSet);
var
  wRec: TBookMark;
begin
  tSHEILD.CommitRetaining;
...
procedure Tfrmrec_can_car.cadastroFIN_PMULValidate(Sender: TField);
begin
  if tag = 0 then
  begin
...
procedure Tfrmrec_can_car.cadastroFIN_PDESValidate(Sender: TField);
begin
  if tag = 0 then
  cadastroFIN_VDES.Value := (cadastroFIN_VALO.AsFloat * cadastroFIN_PDES.AsFloat) / 100;
end;

procedure Tfrmrec_can_car.cadastroFIN_VPAGValidate(Sender: TField);
begin
  if tag = 0 then
  begin
...
procedure Tfrmrec_can_car.dbgConsultaFIN_STFIValidate(Sender: TObject;
  var ErrorText: String; var Accept: Boolean);
begin
...
procedure Tfrmrec_can_car.tsENCShow(Sender: TObject);
begin
  dbgencargos.SetFocus;
  dbgencargos.FocusedColumn := 2;
end;

procedure Tfrmrec_can_car.siTPGClick(Sender: TObject);
begin
  try
    cadastro.DisableControls;
    cadastro.First;
...
procedure Tfrmrec_can_car.SpeedItem1Click(Sender: TObject);
begin
  try
    cadastro.DisableControls;
    cadastro.First;
...
procedure Tfrmrec_can_car.lan_chqAfterDelete(DataSet: TDataSet);
begin
  tSHEILD.CommitRetaining;
end;

procedure Tfrmrec_can_car.dbgConsultaFIN_DPAGValidate(Sender: TObject;
  var ErrorText: String; var Accept: Boolean);
begin
...
procedure Tfrmrec_can_car.dbgrec_recKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  case key of
...
procedure Tfrmrec_can_car.siVINClick(Sender: TObject);
begin
...
procedure Tfrmrec_can_car.siVITDClick(Sender: TObject);
begin
  fin_rec.First;
