...
begin
  oLockWindowUpdate; { Bloqueia Tela }
  oOTransact(TConsulta); { Transações }
end;
...
procedure TFrmSHE_DEF_PED.EdicaoBeforeOpen(DataSet: TDataSet);
begin
  DBGConsulta.BeginUpdate;
end;

procedure TFrmSHE_DEF_PED.EdicaoAfterOpen(DataSet: TDataSet);
begin
...
procedure TFrmSHE_DEF_PED.ACTPSQ_OKExecute(Sender: TObject);
begin
  REC_SHE_DEF.PSQ_OK := (Edicao.RecNo > 0);
  Close;
end;
