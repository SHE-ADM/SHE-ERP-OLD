...
begin
  ...
  REC_SHE_DEF.GRegra      := 'Permissões Gerais';
  oUSER(REC_SHE_DEF);
  ACTMPFinanceiroAppend.Enabled := REC_SHE_DEF.GAppend;
  ACTMPFinanceiroDelete.Enabled := REC_SHE_DEF.GDelete;
  ...
  { CLIENTE }
  ACTMPCAD_CLI_EDI.Enabled := (REC_SHE_DEF.GAppend) or (REC_SHE_DEF.GEdit);
  ACTMPCAD_CLI_CRD.Enabled := (REC_SHE_DEF.GAppend) or (REC_SHE_DEF.GEdit);
...
begin
  ...
  try
    ...
    FrmPesquisa.Caption := 'Pesquisa Pedidos';
    FrmPesquisa.Tag     := 7;
    ...
    try
      ...
      begin
        ...
        SQL.Add('       PK.IDCD,PK.DECD,PK.RZCD,PK.PJCD,PK.PFCD,PK.GPCD,PK.CNCD,PK.CSCD,PK.FIN_VCRD,PK.UFCD,PK.RSCD,');
        ...
        { PESQUISA PERÍODO }
        if FrmPesquisa.DEDTPK.Date > 0 then
        SQL.Add('WHERE ' + FrmPesquisa.IEDATA.Text + ' BETWEEN ''' + FormatDateTime('mm/dd/yy',FrmPesquisa.DEDTPK.Date) + ''' AND ''' + FormatDateTime('mm/dd/yy',FrmPesquisa.DEDTFK.Date) + '''');
        SQL.Add('),'); { RECURSIVE FIM }

        { CTE PSQ }
        if ((FrmPesquisa.EDTXT.Text  <> EmptyStr) and (Pos('PK.',FrmPesquisa.IECAMPO.Text ) > 0)) or
           ((FrmPesquisa.EDTXT2.Text <> EmptyStr) and (Pos('PK.',FrmPesquisa.IECAMPO2.Text) > 0)) then
        begin
          { CTE INI }
          SQL.Add('CTE_PSQ AS (');
          ...
          if (FrmPesquisa.EDTXT.Text <> EmptyStr) and (Pos('PK.',FrmPesquisa.IECAMPO.Text) > 0) then
          SQL.Add('WHERE ' + FrmPesquisa.IECAMPO.Text + ' ' + FrmPesquisa.cPesquisaWhere + ' ''' + FrmPesquisa.EDTXT.Text + FrmPesquisa.cPesquisaLike + '''');

          { PESQUISA TEXTO ADICIONAL }
          if (FrmPesquisa.EDTXT2.Text <> EmptyStr) and (Pos('PK.',FrmPesquisa.IECAMPO2.Text) > 0) then
          begin
            SQL.Add(IFThen(FrmPesquisa.EDTXT.Text = EmptyStr,'WHERE','AND') + '(' + FrmPesquisa.IECAMPO2.Text + ' ' + FrmPesquisa.cPesquisaWhere2 + ' ''' + FrmPesquisa.EDTXT2.Text + FrmPesquisa.cPesquisaLike2 + '''');

            { SITUAÇÕES }
            if Pos('PK.DEST',FrmPesquisa.IECAMPO2.Text) > 0 then
            SQL.Add('OR PK.D_DEST ' + FrmPesquisa.cPesquisaWhere2 + ' ''' + FrmPesquisa.EDTXT2.Text + FrmPesquisa.cPesquisaLike2 + '''');
            SQL.Add(')');
          end;
        ...
        { CTE ADICIONAL FINANCEIRO }
        if (FrmPesquisa.EDTXT2.Text <> EmptyStr) and (Pos('FIN.',FrmPesquisa.IECAMPO2.Text) > 0) then
        begin
          { CTE PSQ }
          if Pos('CTE_PSQ',SQL.Text) > 0 then
          SQL.Add(',');
          ...
          { BANCÁRIO }
          SQL.Add('SELECT PK.* FROM ' + IFThen(Pos('CTE_PSQ',SQL.Text) > 0,'CTE_PSQ','PK') + ' AS PK');
          SQL.Add('JOIN  ' + oREPZero('VW_PSQ_FIN_REC_BAN','_',RECParametros.EP_ID,3)      + ' AS FIN ON (FIN.BXD_CDBX = PK.CDBX)');
          SQL.Add('WHERE ' + FrmPesquisa.IECAMPO2.Text + ' ' + FrmPesquisa.cPesquisaWhere2 + ' ''' + FrmPesquisa.EDTXT2.Text + FrmPesquisa.cPesquisaLike2 + '''');

          { CARTEIRA }
          SQL.Add('UNION');
          SQL.Add('SELECT PK.* FROM ' + IFThen(Pos('CTE_PSQ',SQL.Text) > 0,'CTE_PSQ','PK') + ' AS PK');
          SQL.Add('JOIN  ' + oREPZero('VW_PSQ_FIN_REC_CAR','_',RECParametros.EP_ID,3)      + ' AS FIN ON (FIN.BXD_CDBX = PK.CDBX)');
          SQL.Add('WHERE ' + FrmPesquisa.IECAMPO2.Text + ' ' + FrmPesquisa.cPesquisaWhere2 + ' ''' + FrmPesquisa.EDTXT2.Text + FrmPesquisa.cPesquisaLike2 + '''');
        ...
        { SELECT RESULT }
        SQL.Add('SELECT   DISTINCT PK.* FROM ' + IFThen(Pos('CTE_FIN',SQL.Text) > 0,'CTE_FIN','CTE_PSQ') + ' AS PK');
        SQL.Add('ORDER BY PK.DTCA DESC');
      end;

      if FrmPesquisa.EDTXT.Text <> EmptyStr then
      begin
        APSQ_PDV_PAD := FrmPesquisa.IECAMPO.Text;
        SBRodape.Panels[1].Text := SBRodape.Panels[1].Text + ' ' + FrmPesquisa.IECAMPO.Descriptions[FrmPesquisa.IECAMPO.Values.IndexOf(FrmPesquisa.IECAMPO.Text)] + ' -> ' + FrmPesquisa.EDTXT.Text;
      end;

      if FrmPesquisa.EDTXT2.Text <> EmptyStr then
      begin
        APSQ_PDV_ADC := FrmPesquisa.IECAMPO2.Text;
        SBRodape.Panels[1].Text := SBRodape.Panels[1].Text + ' ' + FrmPesquisa.IECAMPO2.Descriptions[FrmPesquisa.IECAMPO2.Values.IndexOf(FrmPesquisa.IECAMPO2.Text)] + ' -> ' + FrmPesquisa.EDTXT2.Text;
      end;

      if FrmPesquisa.DEDTPK.Date > 0 then
      begin
        APSQ_PDV_ADC := FrmPesquisa.IECAMPO2.Text;
        SBRodape.Panels[1].Text := SBRodape.Panels[1].Text + ' ' + FrmPesquisa.IEDATA.Descriptions[FrmPesquisa.IEDATA.Values.IndexOf(FrmPesquisa.IEDATA.Text)] + ' -> ' +
                                                                   FormatDateTime('dd/mm/yy',FrmPesquisa.DEDTPK.Date) + ' até ' + FormatDateTime('dd/mm/yy',FrmPesquisa.DEDTFK.Date);
      end;
