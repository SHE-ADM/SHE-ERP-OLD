...
uses
  ...
  cxGraphics, cxControls, dxStatusBar, IBSQL, ActnList;
...
type
  Tfrmcad_cli_edi = class(TFrmPadr3)
    ...
    CBRICM: TdxImageEdit;
    Label59: TLabel;
    SIMPSQCadastroFiscal: TSpeedItem;
    cbramo: TComboBox;
    Label56: TLabel;
    PEDEGP: TdxPickEdit;
    IETPRE: TdxImageEdit;
...
begin
  ...
  try
    ...
    ibSP.ParamByName('NAFA').Value := IENAFA.Text;

    { Sintegra }
    ibSP.ParamByName('fant').Value := edfant.Text;
    ibSP.ParamByName('raza').Value := edraza.Text;
    ibSP.ParamByName('DEGP').Value := PEDEGP.Text;
    ...
    { Programações }
    ibSP.ParamByName('tipo'   ).Value := IETPRE.Text;
    ibSP.ParamByName('reserva').Value := IEAVRE.Text;

    { Vendedores }
    ibSP.ParamByName('cven').Value := IEIDCV.Text; { Interno }
    ibSP.ParamByName('crep').Value := IEIDCR.Text; { Externo }
    ...
    ibSP.ParamByName('obse').Value := EMINFADCAD.Text;
    ibSP.ParamByName('obso').Value := EMINFADFIN.Text;

    { Usuário }
    ibSP.ParamByName('dusu').Value := RECUsuarios.Login;

    { Status }
    ibSP.ParamByName('stav').Value := IEREST.Text;
    ...
    oAviso(Application.Handle,'Cliente atualizado com sucesso !');
