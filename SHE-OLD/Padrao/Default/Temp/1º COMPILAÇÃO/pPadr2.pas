...
procedure TFrmPadr2.FormKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key = #5 then
  {[Ctrl+E]} else
...
procedure TFrmPadr2.ACTMAppendExecute(Sender: TObject);
begin
  if Cadastro.State <> dsBrowse then
  Abort;
end;

procedure TFrmPadr2.ACTMEditExecute(Sender: TObject);
begin
  if (Cadastro.State <> dsBrowse) or (Cadastro.RecNo = 0) then
  Abort;
end;

procedure TFrmPadr2.ACTMDeleteExecute(Sender: TObject);
begin
  if (Cadastro.State <> dsBrowse) or (Cadastro.RecNo = 0) then
  Abort;
end;

procedure TFrmPadr2.ACTMPostExecute(Sender: TObject);
begin
  if (Cadastro.State <> dsBrowse) or (Cadastro.RecNo = 0) then
  Abort;
end;

procedure TFrmPadr2.ACTMCancelExecute(Sender: TObject);
begin
  if (Cadastro.State <> dsBrowse) or (Cadastro.RecNo = 0) then
  Abort;
end;
