...
procedure TFrmDefaultEdicao._SetCurrentAlert(const AValue: String);
begin
...
procedure TFrmDefaultEdicao._WM_CREATE(var Msg: TMessage);
begin
  { FORM SCREEN }
...
procedure TFrmDefaultEdicao._WM_AFTER_CREATE(var Msg: TMessage);
begin
  { FORM SCREEN }
...
procedure TFrmDefaultEdicao._WM_ACTIVATE(var Msg: TMessage);
begin
  { EVENTOS }
  { Limpa Eventos }
...
procedure TFrmDefaultEdicao._SW_SHOWNOACTIVATE(var Msg: TMessage);
...
procedure TFrmDefaultEdicao._WM_SHOW(var Msg: TMessage);
begin
  { FORM SHOW }
...
procedure TFrmDefaultEdicao._WM_AFTER_SHOW(var Msg: TMessage);
begin
  { FORM SHOW }
  Screen.Cursor := crHourGlass;
  REC_SHE_DEF.FResize := 0; { FORM RESIZE }
...
procedure TFrmDefaultEdicao._WM_RESIZE(var Message: TMessage);
begin
  { Before Resize }
...
procedure TFrmDefaultEdicao._WM_AFTER_RESIZE(var Message: TMessage);
begin
  { After Resize }
...
procedure TFrmDefaultEdicao.FormCloseQuery(Sender: TObject;
...
procedure TFrmDefaultEdicao.FormKeyPress(Sender: TObject; var Key: Char);
begin
  if key = #5 then
  {[Ctrl+E]} else
  if key = #8 then
...
procedure TFrmDefaultEdicao.ACTRelatoriosExecute(Sender: TObject);
begin
  { nothing }
end;
