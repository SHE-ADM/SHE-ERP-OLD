...
procedure TFrmDefaultConsulta._SetCurrentEvent(const AValue: String);
begin
  FCurrentEvent := AValue;
end;

procedure TFrmDefaultConsulta._SetCurrentAlert(const AValue: String);
begin
  FCurrentAlert := AValue;
end;

procedure TFrmDefaultConsulta._SetForceClose(const AValue: Boolean);
begin
  FForceClose := AValue;
end;

procedure TFrmDefaultConsulta._WM_CREATE(var Msg: TMessage);
begin
  { FORM SCREEN }
...
procedure TFrmDefaultConsulta._WM_AFTER_CREATE(var Msg: TMessage);
begin
  { FORM SCREEN }
...
procedure TFrmDefaultConsulta._WM_ACTIVATE(var Msg: TMessage);
begin
  { EVENTOS }
  { Limpa Eventos }
...
procedure TFrmDefaultConsulta._SW_SHOWNOACTIVATE(var Msg: TMessage);
...
procedure TFrmDefaultConsulta._WM_SHOW(var Msg: TMessage);
begin
  { FORM SHOW }
...
procedure TFrmDefaultConsulta._WM_AFTER_SHOW(var Msg: TMessage);
begin
  { FORM SHOW }
  Screen.Cursor := crHourGlass;
  REC_SHE_DEF.FResize := 0; { FORM RESIZE }
...
procedure TFrmDefaultConsulta._WM_RESIZE(var Message: TMessage);
begin
  { Before Resize }
...
procedure TFrmDefaultConsulta._WM_AFTER_RESIZE(var Message: TMessage);
begin
  { After Resize }
...
Constructor TFrmDefaultConsulta.Create(AOwner: TComponent;
                     const AIDPK     : LongInt = 0 ;
                           ADEPK     : String  = '';
                           AIDEV     : LongInt = 0 ;
                           ACDEV     : Word    = 0 ;
...
Class procedure TFrmDefaultConsulta._ExecForm(AOwner: TComponent;var AForm; AFormPesquisa: Boolean = False; AFormStyle: TFormStyle = fsMDIChild;
                                  AIDPK     : LongInt = 0 ;
                                  ADEPK     : String  = '';
                                  AIDEV     : LongInt = 0 ;
                                  ACDEV     : Word    = 0 ;
...
Destructor TFrmDefaultConsulta.Destroy;
type
  PtrForm = ^TForm;
var
  idxForm: Integer;
...
procedure TFrmDefaultConsulta.FormActivate(Sender: TObject);
begin
  OnActivate := Nil;
  if FForceClose then
  Exit;
...
procedure TFrmDefaultConsulta.FormCloseQuery(Sender: TObject;
  var CanClose: Boolean);
begin
  { VER ANTES DE SAIR }
end;

procedure TFrmDefaultConsulta.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action  := caFree;
  OnClose := Nil;
end;

procedure TFrmDefaultConsulta.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  case key of
...
procedure TFrmDefaultConsulta.FormKeyPress(Sender: TObject; var Key: Char);
begin
  if key = #5 then
  {[Ctrl+E]} else
  if key = #8 then
...
procedure TFrmDefaultConsulta.FormPaint(Sender: TObject);
var
...
procedure TFrmDefaultConsulta.FormResize(Sender: TObject);
begin
  if (not Showing) or (FForceClose) then
  Exit;
...
procedure TFrmDefaultConsulta.CadastroAfterOpen(DataSet: TDataSet);
begin
  Screen.Cursor := crDefault;
  SBRodape.Panels[0].Text := 'Consulta';
end;

procedure TFrmDefaultConsulta.CadastroBeforeOpen(DataSet: TDataSet);
begin
  Screen.Cursor := crAppStart;
end;

procedure TFrmDefaultConsulta.DBGConsultaBackgroundDrawEvent(Sender: TObject;
  ACanvas: TCanvas; ARect: TRect);
var
  Text : String;
begin
...
procedure TFrmDefaultConsulta.ACTRefreshExecute(Sender: TObject);
begin
  oRefresh(Cadastro);
end;

procedure TFrmDefaultConsulta.ACTRelatoriosExecute(Sender: TObject);
begin
  ActiveControl := Nil;
end;

procedure TFrmDefaultConsulta.ACTSaidaExecute(Sender: TObject);
begin
...
procedure TFrmDefaultConsulta.ACTAppendExecute(Sender: TObject);
begin
...
procedure TFrmDefaultConsulta.ACTEditExecute(Sender: TObject);
begin
...
procedure TFrmDefaultConsulta.ACTDeleteExecute(Sender: TObject);
begin
...
procedure TFrmDefaultConsulta.DBGConsultaKeyDown(Sender: TObject;
...
procedure TFrmDefaultConsulta.BEPesquisaKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
...
procedure TFrmDefaultConsulta.ACTPesquisa_RapidaExecute(Sender: TObject);
begin
  BBPesquisa.Caption := TAction(Sender).Caption;
end;

procedure TFrmDefaultConsulta.EEventEventAlert(Sender: TObject; EventName: String;
  EventCount: Integer; var CancelAlerts: Boolean);
begin
  oRefresh(Cadastro);
end;

procedure TFrmDefaultConsulta.EEventAdminEventAlert(Sender: TObject;
...
procedure TFrmDefaultConsulta.ACTExecEventExecute(Sender: TObject);
var
  i: word;
begin
  if REC_SHE_DEF.FB_Event <> EmptyStr then
