...
procedure TFrmDefaultEdicaoGrid._WM_SHOW(var Msg: TMessage);
begin
  { FORM SHOW }
...
procedure TFrmDefaultEdicaoGrid._WM_RESIZE(var Message: TMessage);
begin
  { Before Resize }
...
procedure TFrmDefaultEdicaoGrid.FormKeyPress(Sender: TObject; var Key: Char);
begin
  if key = #5 then
  {[Ctrl+E]} else
  if key = #8 then
