...
type
  Tfrmfin_dup_edi = class(Tfrmpadr3)
    ...
    procedure dtsdup_001StateChange(Sender: TObject);
  private
...
begin
  ...
  REC_SHE_DEF.GReferencia := 'Recebimentos Bancários';
  REC_SHE_DEF.GRegra      := 'Boletos';
  inherited;
...
begin
  ...
  try
    ...
    begin
      ...
      ibSP.ParamByName('nfe').Value  := 'NFE_DUP_'+oStrZero(RECParametros.EP_ID,3);

      ibSP.ParamByName('id').Value    := 0;
      ibSP.ParamByName('CCAB').Value  := psq_romIDN.AsInteger;
      ...
      ibSP.ParamByName('OBSE').Value  := dup_001NFE_OBSE.Text;
...
procedure Tfrmfin_dup_edi.SIMEAppendClick(Sender: TObject);
begin
  if DUP_001.State = dsBrowse then
  DUP_001.Append;
end;

procedure Tfrmfin_dup_edi.SIMEEditClick(Sender: TObject);
begin
  if (DUP_001.State = dsBrowse) and (DUP_001Id.AsInteger > 0) then
  DUP_001.Edit;
end;

procedure Tfrmfin_dup_edi.SIMEDeleteClick(Sender: TObject);
begin
  if (DUP_001.State = dsBrowse) and (DUP_001Id.AsInteger > 0) then
  DUP_001.Delete;
end;

procedure Tfrmfin_dup_edi.SIMEPostClick(Sender: TObject);
begin
  if DUP_001.State in [dsEdit,dsInsert] then
  DUP_001.Post;
end;

procedure Tfrmfin_dup_edi.SIMECancelClick(Sender: TObject);
begin
  if DUP_001.State in [dsEdit,dsInsert] then
  DUP_001.Cancel;
end;

procedure Tfrmfin_dup_edi.dtsdup_001StateChange(Sender: TObject);
begin
  oState(DUP_001,SBMenuEdicao);
end;
