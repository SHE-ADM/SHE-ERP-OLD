...
uses uPrincipal, bPrincipal,
  pFRecebimento_Edicao, prelatorio_geral, pfin_rec_bai,
  pcad_cli_edi, pPesquisa, pPesquisa_OLD;
...
procedure Tfrmfin_rec_con.ACTPesquisaExecute(Sender: TObject);
begin
  ...
  try
    ...
    try
      ...
      begin
        ...
        begin
          if FrmPesquisa_OLD.EDTXT.Text = 'PENDENTE' then
          begin
            SQL.Add('AND PK.DEST NOT LIKE ''PAGO%''');
            SQL.Add('AND PK.DEST <> ''BAIXADO''');
          end else

          if FrmPesquisa_OLD.EDTXT.Text = 'VENCIDO'  then SQL.Add('AND PK.DEST = ''PENDENTE''') else
          if FrmPesquisa_OLD.EDTXT.Text = 'A VENCER' then SQL.Add('AND PK.DEST = ''PENDENTE''') else
          SQL.Add('AND ' + FrmPesquisa_OLD.CField + ' LIKE ''' + FrmPesquisa_OLD.EDTXT.Text + '%''');
        end;

        if (FrmPesquisa_OLD.dxDT1.Date > 0) and (FrmPesquisa_OLD.dxDT2.Date > 0) then
        SQL.Add('AND ' + FrmPesquisa_OLD.cData + ' BETWEEN ''' + formatDateTime('mm/dd/yy',FrmPesquisa_OLD.dxDT1.Date) + ''' AND ''' + formatDateTime('mm/dd/yy',FrmPesquisa_OLD.dxDT2.Date) + '''');
      ...
      if FrmPesquisa_OLD.EDTXT.Text <> EmptyStr then
      begin
        APSQ_FIN_REC := FrmPesquisa_OLD.cbCAMPO.Text;
...
procedure Tfrmfin_rec_con.CadastroAfterClose(DataSet: TDataSet);
begin
  inherited;
  SBRodape.Panels[1].Text := EmptyStr;
end;

procedure Tfrmfin_rec_con.SICAD_CLI_INFClick(Sender: TObject);
begin
  if CadastroId.AsInteger = 0 then
