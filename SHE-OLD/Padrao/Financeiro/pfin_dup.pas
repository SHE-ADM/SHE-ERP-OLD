...
uses
  ...
  IBEvents, dxExEdtr, StdCtrls, rxSpeedbar, IBSQL, ActnList;
...
uses uPrincipal,
  pPesquisa_OLD, prelatorio_geral, pfin_dup_edi;
...
begin
  ...
  try
    FrmPesquisa_OLD.Tag          := 11;
    FrmPesquisa_OLD.cbCAMPO.Text := 'Nº Nota Fiscal';
    FrmPesquisa_OLD.cbDATA.Text  := '';
    FrmPesquisa_OLD.ShowModal;

    if FrmPesquisa_OLD.editado then
    with FrmPesquisa_OLD,cadastro do
    begin
      SelectSQL.Clear;
      SelectSQL.Add('SELECT   NFE_DUP.*,NFE_CAB.NFE_VNF,NFE_CAB.NFE_CHAV,');
      SelectSQL.Add('         CAD_CLI.CLI_FANT,CAD_CLI.CLI_RAZA,CAD_CLI.CLI_DDD,CAD_CLI.CLI_TEL1,CAD_CLI.CLI_CNPJ,CAD_CLI.CLI_INSC,');
