    ...
    begin
      ...
      if EdicaoFLAG.AsInteger = 0 then
         ...
         begin
           { Inicializar Parâmetros: FLAG + campos de pesquisa (IDEP,IDPK,IDFK) }
           FLAG := 0;
           CDET := EdicaoCDET.AsInteger; { Etiqueta }

           { Romaneio }
           ITEM := EdicaoITEM.AsInteger;

           { Produtos }
           IDCP    := EdicaoIDCP.AsInteger;
           CP_IDEP := EdicaoCP_IDEP.AsInteger;
           IDAK    := EdicaoIDAK.AsInteger;
           CEAN    := EdicaoCEAN.AsString;

           { Lançamentos }
           QTDE := EdicaoQTDE.AsFloat;
           QTRL := EdicaoQTRL.AsInteger;

           { Custo }
           CF_VPRC_PAD_ORI := EdicaoCF_VPRC_PAD_ORI.AsString;
           CF_VPRC_PAD     := EdicaoCF_VPRC_PAD.AsFloat;
           CF_VPRC_COM     := EdicaoCF_VPRC_COM.AsFloat;

           { Classificações }
           COL_ID := EdicaoCOL_ID.AsInteger; { Coleção - Compras }
           NFCI   := EdicaoNFCI.AsString; { Ficha de Conteúdo de Importação }

           { Defeitos }
           IDDF := IFThen(EdicaoEST_CDDF.AsInteger > 0,RECUsuarios.ID,0);
           CDDF := EdicaoEST_CDDF.AsInteger;

           { Informações Adicionais }
           INFADCAD := EdicaoINFADTEC.AsString;

           { Registros }
           IDEV := 0;

           oSP_CAD_PRO_EST_RFK_OLD(RECEdicao); { Execute Procedure }
         end;
