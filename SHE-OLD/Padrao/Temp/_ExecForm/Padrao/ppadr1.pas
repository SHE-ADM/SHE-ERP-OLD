...
type
  TFrmPadr1 = class(TForm)
    ...
    PNLPrincipalLE: TPanel;
    PNLPrincipalLD: TPanel;
...
Class procedure TFrmPadr1._ExecForm(AOwner: TComponent;var AForm; AFormPesquisa: Boolean = False; AFormStyle: TFormStyle = fsMDIChild;
                                    AIDPK : LongInt = 0 ;
                                    ADEPK : String  = '';
                                    AIDEV : LongInt = 0 ;
                                    ACDEV : Word    = 0 ;
...
procedure TFrmPadr1.ACTConsultaExecute(Sender: TObject);
begin
  { INICIALIZAÇÃO DE TRANSAÇÕES }
  oOTransact(TEdicao); { Edições }
...
procedure TFrmPadr1.ACTEdicaoExecute(Sender: TObject);
begin
  { INICIALIZAÇÃO DE TRANSAÇÕES }
end;

procedure TFrmPadr1.ACTMPAppendExecute(Sender: TObject);
begin
  ActiveControl := Nil;
end;

procedure TFrmPadr1.ACTMPEditExecute(Sender: TObject);
begin
  ActiveControl := Nil;
end;

procedure TFrmPadr1.ACTMPDeleteExecute(Sender: TObject);
begin
  ActiveControl := Nil;
end;

procedure TFrmPadr1.ACTMPPostExecute(Sender: TObject);
begin
  ActiveControl := Nil;
end;

procedure TFrmPadr1.ACTMPCancelExecute(Sender: TObject);
begin
  ActiveControl := Nil;
end;
...
procedure TFrmPadr1.ACTCheckConstraintsExecute(Sender: TObject);
begin
  ActiveControl := Nil;
end;

procedure TFrmPadr1.ACTCheckErrorsExecute(Sender: TObject);
begin
  ActiveControl := Nil;
end;
...
procedure TFrmPadr1.ACTDashboardsExecute(Sender: TObject);
begin
  ActiveControl := Nil;
end;

procedure TFrmPadr1.ACTProgressBarExecute(Sender: TObject);
begin
  ActiveControl := Nil;
end;

procedure TFrmPadr1.CadastroBeforeOpen(DataSet: TDataSet);
begin
  { INICIALIZA FORM SCREEN }
  Screen.Cursor := crHourGlass;
end;
...
begin
  Cadastro.FieldByName('ID').Value := 0;

  if not ALockWindowUpdate then
  REC_SHE_DEF.Editing := True; { Criação }
end;

procedure TFrmPadr1.CadastroBeforePost(DataSet: TDataSet);
var
  i: Word;
begin
  if not ALockWindowUpdate then
  begin
    { VER MODIFICAÇÃO NOS REGISTROS }
    for i := 0 to Cadastro.FieldCount - 1 do
    begin
      if Cadastro.Fields[i].NewValue <> Cadastro.Fields[i].OldValue then
      begin
        ...
        REC_SHE_DEF.Editing := True; { Modificado }
        Break;
      end;
    end;
{ ricardo ver criação }
...
procedure TFrmPadr1.CadastroEditError(DataSet: TDataSet; E: EDatabaseError;
  var Action: TDataAction);
begin
  DataBaseError(oFBException(E.Message));
end;

procedure TFrmPadr1.CadastroDeleteError(DataSet: TDataSet;
...
procedure TFrmPadr1.CadastroCalcFields(DataSet: TDataSet);
begin
  CadastroC_ID.Value := Cadastro.RecNo;
end;

procedure TFrmPadr1.DTSCadastroDataChange(Sender: TObject; Field: TField);
begin
  { nothing }
end;
...
begin
  ...
  begin
    { MENU PRINCIPAL }
    ACTRefresh.Enabled    := not (Cadastro.State in [dsInsert,dsEdit]);
    ACTPesquisa.Enabled   := not (Cadastro.State in [dsInsert,dsEdit]);
    ACTRelatorios.Enabled := not (Cadastro.State in [dsInsert,dsEdit]);
    ACTSaida.Enabled      := not (Cadastro.State in [dsInsert,dsEdit]);

    { MENU DE EDIÇÕES }
    oState(Cadastro,SBMenuEdicao);

    { RODATÉ }
    SBRodape.Panels[0].Text := IFThen(Cadastro.State = dsInsert  ,'Incluindo ...',
                               IFThen(Cadastro.State = dsEdit    ,'Editando ...' ,
                               IFThen(Cadastro.State = dsInactive,'Inativo','Consulta')));
...
procedure TFrmPadr1.DBGConsultaCustomDrawCell(Sender: TObject;
...
begin
  case key of
       vk_return: if Cadastro.State in [dsInsert,dsEdit] then
                  begin
                    DBGConsulta.FocusedColumn   := DBGConsulta.FocusedColumn + 1;
                    if DBGConsulta.FocusedColumn > DBGConsulta.ColumnCount   - 1 then
                    begin
                      Cadastro.Next;
...
procedure TFrmPadr1.ACTEveRegisterExecute(Sender: TObject);
begin
  { UNREGISTER EVENTS }
  if EEvent.Registered then
     try
...
procedure TFrmPadr1.ACTEveExecuteExecute(Sender: TObject);
var
  i: word;
begin
  if REC_SHE_DEF.FB_Event = EmptyStr then
...
procedure TFrmPadr1.ACTEveExpressExecute(Sender: TObject);
begin
  ACTEveRegister.Execute;
  ACTEveExecute.Execute;
end;
