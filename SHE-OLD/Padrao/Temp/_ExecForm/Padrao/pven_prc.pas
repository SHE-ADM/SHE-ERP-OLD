...
procedure TFrmVEN_PRC._WM_CREATE(var Msg: TMessage);
begin
...
procedure TFrmVEN_PRC._WM_AFTER_CREATE(var Msg: TMessage);
begin
  { INICIALIZAÇÃO }
...
procedure TFrmVEN_PRC._SW_SHOWNOACTIVATE(var Msg: TMessage);
...
procedure TFrmVEN_PRC._WM_AFTER_SHOW(var Msg: TMessage);
begin
  ABRE_TABELA;
...
procedure TFrmVEN_PRC._WM_ACTIVATE(var Msg: TMessage);
begin
  { EVENTOS }
  { Limpa Eventos }
...
procedure TFrmVEN_PRC._WM_AFTER_ACTIVATE(var Msg: TMessage);
begin
  try
...
procedure TFrmVEN_PRC._WM_BEFORE_RESIZE(var Message: TMessage);
begin
  { Before Resize }
  DBGEdicao.Tag := DBGEdicao.Tag + 1;
...
procedure TFrmVEN_PRC._WM_AFTER_RESIZE(var Message: TMessage);
begin
  { After Resize }
  if DBGEdicao.Tag >= 2 then
     try
       oResize(DBGEdicao);
       Paint;
     finally
       DBGEdicao.Tag := 0; { zera controle }
