...
procedure TFrmETI_PRO._WM_AFTER_ACTIVATE(var Msg: TMessage);
begin
  inherited;
  with SQLConsulta do
  begin
...
begin
  ...
  { ACCESS MANAGER }
  REC_SHE_DEF.FB_Event := 'CAD_PRO_EST_ADM'; { Eventos }
...
begin
//ricardo   siVIS.Enabled := false;
//ricardo   siALT.Enabled := bPSQUSER('USU_AUTO','Produtos','Estoque','Alterar Estoque');
//ricardo   siCSE.Enabled := bPSQUSER('USU_AUTO','Produtos','Estoque','Cancelar Separação');
//ricardo   siCAN.Enabled := bPSQUSER('USU_AUTO','Produtos','Estoque','Cancelar Estoque');

  if laolan.Caption = 'ENTRADA' then
  begin
//ricardo     siVIS.Enabled := true;
 //ricardo    siALT.Enabled := true;
//ricardo     siCAN.Enabled := true;
  end;
...
//    siPRI.Enabled := false;
//    siVIS.Enabled := false;
 //ricardo    siALT.Enabled := false;
 //ricardo    siCAN.Enabled := false;
 //ricardo    siCSE.Enabled := true;
  end;

  if laolan.Caption = 'FATURADO' then
  begin
 //ricardo    siPRI.Enabled := false;
 //ricardo    siVIS.Enabled := false;
 //ricardo    siALT.Enabled := false;
  //ricardo   siCAN.Enabled := false;
 //ricardo    siCSE.Enabled := false;
  end else
 //ricardo  siCSE.Enabled := (lanped.Tag > 0);
end;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        ASPEdicao := SPEdicao; { Stored Procedure }
...
begin
 ...
 //ricardo      siCSE.Enabled  :=(Current.ByName('IDPK').AsInteger > 0);
