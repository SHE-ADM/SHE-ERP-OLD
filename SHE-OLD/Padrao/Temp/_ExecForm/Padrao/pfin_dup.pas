...
uses
  ...
  IBEvents, dxExEdtr, StdCtrls, rxSpeedbar, IBSQL, ActnList, cxGraphics,
...
uses uPrincipal,
  ppesquisa_geral, prelatorio_geral, pfin_dup_edi;

{$R *.dfm}

procedure Tfrmfin_dup._WM_AFTER_ACTIVATE(var Msg: TMessage);
begin
  inherited;
  with cadastro do
  begin
...
begin
  inherited;
  frmpesquisa_geral := Tfrmpesquisa_geral.Create(Nil);
  try
    frmpesquisa_geral.Tag          := 6;
    frmpesquisa_geral.cbCAMPO.Text := 'Nota Fiscal';
    frmpesquisa_geral.cbDATA.Text  := 'Emissão';
    frmpesquisa_geral.ShowModal;

    if frmpesquisa_geral.editado then
    with frmpesquisa_geral,cadastro do
    begin
      SelectSQL.Clear;
      SelectSQL.Add('SELECT   NFE_DUP.*,NFE_CAB.NFE_VNF,NFE_CAB.NFE_CHAV,');
      SelectSQL.Add('         CAD_CLI.CLI_FANT,CAD_CLI.CLI_RAZA,CAD_CLI.CLI_DDD,CAD_CLI.CLI_TEL1,CAD_CLI.CLI_CNPJ,CAD_CLI.CLI_INSC,');
...
begin
  ...
  try
    ...
    if frmfin_dup_edi.REC_SHE_DEF.Edited then
    begin
      with cadastro do
      begin
        SelectSQL.Clear;
