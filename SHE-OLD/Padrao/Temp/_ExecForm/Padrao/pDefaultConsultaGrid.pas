...
procedure TFrmDefaultConsultaGrid._WM_AFTER_ACTIVATE(var Msg: TMessage);
begin
  Screen.Cursor := crDefault;
...
procedure TFrmDefaultConsultaGrid._WM_BEFORE_RESIZE(var Message: TMessage);
begin
  { Before Resize }
  AResize := AResize + 1;
...
procedure TFrmDefaultConsultaGrid.FormKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key = #5 then
  {[Ctrl+E]} else
