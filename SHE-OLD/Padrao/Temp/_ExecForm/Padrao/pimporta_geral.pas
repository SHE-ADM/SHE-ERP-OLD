...
type
  Tfrmimporta_geral = class(Tfrmpadr3)
    ...
    procedure dbgNFE_ITEKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure siVALDblClick(Sender: TObject);
  private
...
procedure TFrmIMPORTA_GERAL._WM_AFTER_ACTIVATE(var Msg: TMessage);
begin
  inherited;
  with consulta do
  begin
  ...
  cbfant.Text := RECParametros.EP_NO;
...
begin
  ...
  result := RECParametros.SHE_DATA else
  ...
  repeat
    ...
    repeat
      ...
      begin
        ...
        if NodeImp.ChildNodes[0].NodeName = 'ICMSST' then
        begin
          // Lendo o nó dos ICMS
          NodeIMPOSTO := NodeImp.ChildNodes['ICMSST'];
...
begin
  ...
  SBRodape.Hint := copy(NodeSec.ChildNodes['dEmi'].Text ,9,2)+'/'+copy(NodeSec.ChildNodes['dEmi'].Text, 6,2)+'/'+copy(NodeSec.ChildNodes['dEmi'].Text,1,4);
  if Length(SBRodape.Hint) < 8 then
  SBRodape.Hint := copy(NodeSec.ChildNodes['dhEmi'].Text,9,2)+'/'+copy(NodeSec.ChildNodes['dhEmi'].Text,6,2)+'/'+copy(NodeSec.ChildNodes['dhEmi'].Text,1,4);

  imp_ncaNFE_DEMI.Value := strtodate(SBRodape.Hint);
...
begin
  ...
  try
    ...
    begin
      if uPesquisa_Produto(RECParametros.EP_ID,pro_iteIDCP.AsString,'ID') > 0 then
      begin
        imp_ite.Append;
        ...
        begin
          ...
          if cbTIPO.Tag > 0 then
             ...
             begin
               ...
               SQL.Add('LEFT  JOIN '+oREPZero('ROM_ITE'    ,'_',RECParametros.EP_ID,3)+' AS RK ON (RK.IDPK = PK.CDRO AND RK.IDCP = FK.IDCP)');
...
begin
  ...
  try
    ...
    begin
      if uPesquisa_Produto(4,RECParametros.EP_ID,consulta.Fields[0].AsString,'ID') > 0 then
      begin
        imp_ite.Append;
...
begin
  ...
  try
    ...
    begin
      if uPesquisa_Produto(RECParametros.EP_ID,prg_iteROM_CPRO.AsString,'ID') > 0 then
      begin
...
begin
  ...
  try
    ...
    begin
      if uPesquisa_Produto(RECParametros.EP_ID,ped_iteROM_CPRO.AsString,'ID') > 0 then
      begin
...
begin
  ...
  try
    ...
    begin
      if uPesquisa_Produto(RECParametros.EP_ID,com_iteROM_CPRO.AsString,'ID') > 0 then
      begin
        imp_ite.Append;
...
begin
  ...
  try
    ...
    begin
      if uPesquisa_Produto(RECParametros.EP_ID,rom_iteIDCP.AsString,'ID') > 0 then
      begin
        imp_ite.Append;
...
procedure Tfrmimporta_geral.siVALDblClick(Sender: TObject);
begin
  if cbTIPO.Text = '' then
