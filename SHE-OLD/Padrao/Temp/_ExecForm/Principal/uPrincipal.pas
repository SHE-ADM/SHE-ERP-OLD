...
type
  ...
  procedure uPesquisaGeral(var AREC_SHE_DEF: TREC_SHE_DEF); STDCALL;
  procedure uPSQ_CAD_PAD(var AREC_SHE_DEF: TREC_SHE_DEF); STDCALL;
  procedure uPSQ_CAD_PRO(var AREC_SHE_DEF: TREC_SHE_DEF); STDCALL;

  procedure uEDT_CAD_PAD(var AREC_SHE_DEF: TREC_SHE_DEF); STDCALL;
...
var
  ...
  APSQ_PED_PRG,APSQ_PED_VEN,APSQ_ROM_CAB,APSQ_NFE,APSQ_REC,APSQ_CAD: String;
...
uses bPrincipal,
  ...
  pRelatorios, pcad_cli_inf, pSenha, pImpressoras,
  pPesquisa, pemail, pven_nfe,
  ...
  pEXP_SEP_MAN, pEXP_SEP_COL, pSHE_DEF_PSQ,
  pPSQ_CAD_PAD, pNFeConsulta, pProduto_;
...
procedure uPesquisaGeral(var AREC_SHE_DEF: TREC_SHE_DEF); STDCALL;
begin
  if AREC_SHE_DEF.FB_PSQ_CD > 0 then { Identificado }
    try

      { EXECUTA PESQUISA }
      FrmPesquisa_Geral     := TFrmPesquisa_Geral.Create(Application);
      FrmPesquisa_Geral.Tag := AREC_SHE_DEF.FB_PSQ_CD;
      FrmPesquisa_Geral.ShowModal;

      if FrmPesquisa_Geral.Editado then
      begin
        { TEXTO }
        if (FrmPesquisa_Geral.CField <> EmptyStr) and (FrmPesquisa_Geral.EDTXT.Text <> EmptyStr) then
        begin
          AREC_SHE_DEF.FB_PSQ_FD_NO_PK := TRIM(FrmPesquisa_Geral.CField); { Field }
          AREC_SHE_DEF.FB_PSQ_FD_VD_PK := TRIM(FrmPesquisa_Geral.EDTXT.Text); { Valor }

          AREC_SHE_DEF.FB_PSQ_WHERE := 'LIKE';
          AREC_SHE_DEF.FB_PSQ_LKPK  := '';
          AREC_SHE_DEF.FB_PSQ_LKFK  := '%';

          AREC_SHE_DEF.FB_PSQ_FD_NO_PK := AREC_SHE_DEF.FB_PSQ_FD_NO_PK + ' ' + AREC_SHE_DEF.FB_PSQ_WHERE + ' ' + AREC_SHE_DEF.FB_PSQ_LKPK + '''' + AREC_SHE_DEF.FB_PSQ_FD_VD_PK + AREC_SHE_DEF.FB_PSQ_LKFK + '''';
        end;

        { DATAS }
        if (FrmPesquisa_Geral.cData <> EmptyStr) and (FrmPesquisa_Geral.dxDT1.Date > 0) and (FrmPesquisa_Geral.dxDT2.Date >= FrmPesquisa_Geral.dxDT1.Date) then
        begin
          AREC_SHE_DEF.FB_PSQ_DT_NO_PK := TRIM(FrmPesquisa_Geral.cData); { Field }
          AREC_SHE_DEF.FB_PSQ_DT_VD_PK := FrmPesquisa_Geral.dxDT1.Date;  { Valor Inicial }
          AREC_SHE_DEF.FB_PSQ_DT_VD_FK := FrmPesquisa_Geral.dxDT2.Date;  { Valor Final }

          AREC_SHE_DEF.FB_PSQ_DT_NO_PK := AREC_SHE_DEF.FB_PSQ_DT_NO_PK + ' BETWEEN ''' + FormatDateTime('mm/dd/yy',AREC_SHE_DEF.FB_PSQ_DT_VD_PK) + ''' AND ''' + FormatDateTime('mm/dd/yy',AREC_SHE_DEF.FB_PSQ_DT_VD_FK) + '''';
        end;
      end;

    finally

      if (AREC_SHE_DEF.FB_PSQ_FD_NO_PK = EmptyStr) and (AREC_SHE_DEF.FB_PSQ_DT_NO_PK = EmptyStr) then
      begin
        AREC_SHE_DEF.FB_PSQ_OK := False;
        Abort;
      end else
      begin
        AREC_SHE_DEF.FB_PSQ_LB := FrmPesquisa_Geral.cbCAMPO.Text;
...
//    TFrmPesquisaCodigoMunicipal._ExecForm(Nil,FrmPesquisaCodigoMunicipal,fsNormal,RECEndereco.CMUN,-1,RECEndereco.UF);
    ...
    if REC_SHE_DEF.Selected then
       RECEndereco.CMUN := REC_SHE_DEF.FB_PSQ_TB_NO_PK;
  end;
end;
...
//ricardo  TFrmCAD_CLI.__ExecForm(
//
//  Application, { Owner    }
//  FrmCAD_CLI,  { Form     }
//  False,       { Pesquisa }
...
begin
//ricardo  TFrmcad_rep.__ExecForm(Nil,Frmcad_rep);
end;

procedure TFrmPrincipal.ACTTransportadorasExecute(Sender: TObject);
begin
//ricardo   TFrmcad_tra.__ExecForm(Nil,Frmcad_tra);
end;

procedure TFrmPrincipal.ACTFornecedoresExecute(Sender: TObject);
begin
//ricardo   TFrmcad_for.__ExecForm(Nil,Frmcad_for);
end;

procedure TFrmPrincipal.ACTICMS_STExecute(Sender: TObject);
begin
//  TFrmICMS_ST.__ExecForm(Nil,FrmICMS_ST);
end;

procedure TFrmPrincipal.ACTSegmentosExecute(Sender: TObject);
begin
//ricardo  TFrmSegmentos._ExecForm(Nil,FrmSegmentos);
end;

procedure TFrmPrincipal.ACTGradeExecute(Sender: TObject);
begin
//ricardo  TFrmProduto_Cor._ExecForm(Nil,FrmProduto_Cor);
end;

procedure TFrmPrincipal.ACTGruposExecute(Sender: TObject);
begin
//ricardo  TFrmProduto_Grupo._ExecForm(Nil,FrmProduto_Grupo);
end;

procedure TFrmPrincipal.ACTSubGruposExecute(Sender: TObject);
begin
//ricardo  TFrmProduto_SubGrupo._ExecForm(Nil,FrmProduto_SubGrupo);
end;

procedure TFrmPrincipal.ACTCategoriasExecute(Sender: TObject);
begin
//ricardo  TFrmProduto_Categoria._ExecForm(Nil,FrmProduto_Categoria);
end;

procedure TFrmPrincipal.ACTSubCategoriasExecute(Sender: TObject);
begin
//ricardo  TFrmProduto_SubCategoria._ExecForm(Nil,FrmProduto_SubCategoria);
end;

procedure TFrmPrincipal.ACTDEstoqueExecute(Sender: TObject);
begin
//ricardo  TFrmEstoqueDefeitos._ExecForm(Nil,FrmEstoqueDefeitos);
end;
...
begin
//ricardo  TFrmEstoque._ExecForm(Nil,FrmEstoque);
end;

procedure TFrmPrincipal.ACTMExpedicaoExecute(Sender: TObject);
begin
//ricardo  FrmEXP_SEP_MAN := TFrmEXP_SEP_MAN.Create(Application);
//ricardo  FrmEXP_SEP_MAN.Show;
end;

procedure TFrmPrincipal.ACTExpedicaoExecute(Sender: TObject);
begin
//ricardo  TFrmEXP_SEP_COL._ExecForm(Nil,FrmEXP_SEP_COL);
end;

procedure TFrmPrincipal.ACTAberturaExecute(Sender: TObject);
begin
//ricardo  FrmCAI_ABR := TFrmCAI_ABR.Create(Application);
end;
...
//ricardo   if Assigned(FrmCTR_PED) then FrmCTR_PED.BringToFront else
//   begin
//     TFrmCTR_PED._ExecForm(
//
//     Application, { Owner }
//     FrmCTR_PED); { Form  }
//   end;
...
begin
  if Assigned(FrmFIN_DUP) then FrmFIN_DUP.BringToFront else
  begin
    FrmFIN_DUP := TFrmFIN_DUP.Create(Self);
    FrmFIN_DUP.Show;
  end;
end;
...
begin
  TFrmProduto_._ExecForm(Nil,FrmProduto_);
end;
          ...
          begin
            try
              ...
              begin
                ...
                SQL.Add('WHERE    CAST(PK.DTBX AS DATE) >= ''10/01/25''');
...
begin
  ...
  try
    ...
    begin
      ...
      ODExplorer.InitialDir := RECParametros.PATH_SHE_DESKTOP;
