unit qGGerencial_Performance_Venda;
...
type
  TqrpGGerencial_Performance_Venda = class(TQuickRep)
    QRBDetalhe: TQRBand;
    QRGGrupo: TQRGroup;
...
var
  qrpGGerencial_Performance_Venda: TqrpGGerencial_Performance_Venda;
...
Constructor TqrpGGerencial_Performance_Venda.Create(AOwner: TComponent; const ARECRelatorio: TRECRelatorios);
begin
...
procedure TqrpGGerencial_Performance_Venda.WinControlFormCreate(Sender: TObject);
          procedure _Report;
          begin
          ...
          begin
            ...
            if RECRelatorio.Tipo = 'Diário' then
               SQLAdd := 'LPad(Extract(Day  FROM PK.ROM_DROM),2,0)||''/''||LPad(Extract(Month FROM PK.ROM_DROM),2,0)||''/''||LPad(Extract(Year FROM PK.ROM_DROM),4,0) AS PERIODO,'
            else
...
begin
  with qrpGGerencial_Performance_Venda do
    try
      _Select;
      _Report;
      ...
      WinControlFormDestroy(qrpGGerencial_Performance_Venda);
    end;
end;

procedure TqrpGGerencial_Performance_Venda.WinControlFormDestroy(Sender: TObject);
begin
  try
    ...
    FreeAndNil(qrpGGerencial_Performance_Venda);
  end;
end;

procedure TqrpGGerencial_Performance_Venda.RelatorioCalcFields(
  DataSet: TDataSet);
begin
...
procedure TqrpGGerencial_Performance_Venda.QRBDetalheBeforePrint(
