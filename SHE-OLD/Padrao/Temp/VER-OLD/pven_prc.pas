...
type
  Tfrmven_prc = class(TForm)
    ...
    DBGEdicaoROM_CTNR: TdxDBGridMaskColumn;
  ...
  private
    ...
    procedure _FillCP;
    procedure _ExecEvent(AIDLG: Word = 0);
  public
...
procedure Tfrmven_prc.SetMSGRodape(const AValue: String);
begin
  FMSGRodape := AValue;
end;

procedure Tfrmven_prc.SetCaixa(const AValue: Boolean);
begin
  FCaixa := AValue;
end;
...
begin
  ...
  CEIDCD.Value := RECParametros.IDCF_PAD;
  EDDECD.Text  := RECParametros.DECF_PAD;
  edccom.Text  := RECUsuarios.Id;
  cbdcom.Text  := RECUsuarios.Login;
  edcrep.Text  := RECParametros.IDCR_PAD;
  cbdrep.Text  := RECParametros.DECR_PAD;
  edstpd.Text  := 'COMPRAS';
  cbstpd.Text  := 'COMPRAS';
  cbstco.Text  := RECParametros.PED_STCO;
  edcpag.Text  := RECParametros.PED_CDPG;
  cbdpag.Text  := RECParametros.PED_DEPG;
...
begin
  ...
  begin
    ...
    Tag := 1;
    IEIDEP.Text      := frmctr_prc.cadastroIDEP.AsString;
    CEIDCD.Value     := frmctr_prc.cadastroROM_CFOR.AsInteger;
    EDDECD.Text      := frmctr_prc.cadastroDECF.AsString;
    EDCNPJ.Text      := frmctr_prc.cadastroFOR_CNPJ.AsString;
    ...
    DEDROM.Date      := frmctr_prc.cadastroROM_DROM.AsDateTime;
    DEDEXP.Date      := frmctr_prc.cadastroROM_DEXP.AsDateTime;
    ...
    IEMFRT.Text      := frmctr_prc.cadastroROM_MFRT.AsString;
    PEDECT.Tag       := frmctr_prc.cadastroROM_CTRA.AsInteger;
    PEDECT.Text      := frmctr_prc.cadastroROM_DTRA.AsString;
    PECTNR.Text      := frmctr_prc.cadastroROM_CTNR.AsString;
    CEVFRT.Value     := frmctr_prc.cadastroROM_VFRT.AsFloat;
    LATDesc.Caption  := frmctr_prc.cadastroROM_TDSC.AsString;
    CEPDesc.Value    := frmctr_prc.cadastroROM_PDSC.AsFloat;

    PESQUISA_FPAGTO('C',frmctr_prc.cadastroROM_CPAG.AsString);

    Tag := 1;
    try
      frmctr_prc.rom_ite.DisableControls;
      ...
      begin
        Edicao.Append;
        uPesquisa_Produto(RECParametros.Id,frmctr_prc.rom_iteIDCP.AsString,'ID');
        _FillCP;

        EdicaoROM_ITEM.Value := oStrZero(frmctr_prc.rom_iteITEM.AsInteger,5);
        EdicaoROM_IPRO.Value := frmctr_prc.rom_iteIDCP.AsInteger;
        EdicaoROM_DPRO.Value := frmctr_prc.rom_iteDECP.AsString;
        EdicaoROM_GCOR.Value := frmctr_prc.rom_iteROM_NFCI.AsString;
        EdicaoROM_DCO2.Value := frmctr_prc.rom_iteDGCP.AsString;
        EdicaoROM_CSEP.Value := frmctr_prc.rom_iteIDCOL.AsInteger;
        EdicaoROM_DSEP.Value := frmctr_prc.rom_iteDECOL.AsString;
        EdicaoROM_DUNI.Value := frmctr_prc.rom_iteROM_DUNI.AsString;
        EdicaoROM_QTDE.Value := frmctr_prc.rom_iteROM_QTDE.AsFloat;
        EdicaoROM_QTRL.Value := frmctr_prc.rom_iteROM_QTRL.AsInteger;
        EdicaoROM_QTPD.Value := frmctr_prc.rom_iteROM_QTPD.AsFloat;
        EdicaoROM_RLPD.Value := frmctr_prc.rom_iteROM_RLPD.AsInteger;
        EdicaoROM_CTNR.Value := frmctr_prc.rom_iteROM_CTNR.AsString;

        EdicaoROM_PREC.AsFloat := frmctr_prc.rom_iteROM_PREC.AsFloat;
        EdicaoROM_UNIT.AsFloat := frmctr_prc.rom_iteROM_UNIT.AsFloat;

        EdicaoROM_CDPD.Value := frmctr_prc.rom_iteROM_CDPD.AsInteger;
        if EdicaoROM_CDPD.AsInteger > 0 then
        EdicaoROM_CDRO.Value := 1 else
        EdicaoROM_CDRO.Value := 0;

        EdicaoROM_QPRD.Value := frmctr_prc.rom_iteROM_QPRD.AsInteger;
        EdicaoROM_QEMB.Value := frmctr_prc.rom_iteROM_QEMB.AsInteger;
        EdicaoROM_QEXP.Value := frmctr_prc.rom_iteROM_QEXP.AsInteger;
        EdicaoROM_QDES.Value := frmctr_prc.rom_iteROM_QDES.AsInteger;

        EdicaoROM_DROM.Value := frmctr_prc.rom_iteROM_DROM.AsDateTime;
        EdicaoROM_DPRD.Value := frmctr_prc.rom_iteROM_DPRD.AsDateTime;
        EdicaoROM_RPRD.Value := frmctr_prc.rom_iteROM_RPRD.AsDateTime;
        EdicaoROM_DEMB.Value := frmctr_prc.rom_iteROM_DEMB.AsDateTime;
        EdicaoROM_REMB.Value := frmctr_prc.rom_iteROM_REMB.AsDateTime;
        EdicaoROM_DDES.Value := frmctr_prc.rom_iteROM_DDES.AsDateTime;
        EdicaoROM_RDES.Value := frmctr_prc.rom_iteROM_RDES.AsDateTime;
        EdicaoROM_DEXP.Value := frmctr_prc.rom_iteROM_DEXP.AsDateTime;
        EdicaoROM_DBAI.Value := frmctr_prc.rom_iteROM_DBAI.AsDateTime;
        EdicaoROM_STFI.Value := frmctr_prc.rom_iteROM_STAV.AsString;

        Edicao.Post;
        frmctr_prc.rom_ite.Next;
      end;
    finally
      frmctr_prc.rom_ite.EnableControls;
  ...
  begin
    ...
    PESQUISA_REPRESENTANTE('C',RECParametros.IDCR_PAD);
...
begin
  ...
  try
    ...
    try
      UnregisterEvents;
      Events.Add(TRIM(oREPZero(REC_SHE_DEF.FB_Event,'_',RECUsuarios.ID,3)));
      RegisterEvents;
    except
      on E: Exception do
      begin
...
{    DBGEdicaoROM_CPRO.DisableEditor := (not oEmpty(EdicaoROM_QTPD.AsFloat));
    DBGEdicaoROM_DCO2.DisableEditor := DBGEdicaoROM_CPRO.DisableEditor;
...
procedure Tfrmven_prc.IFotoClick(Sender: TObject);
begin
  uLoadZoom(IFoto.Picture,[EdicaoROM_CART.AsString,
                           EdicaoROM_CPRO.AsString,
                           EdicaoROM_DCO2.AsString,
...
begin
  ...
  try
    ...
    begin
      ...
      ibSP.ParamByName('ped' ).Value := oREPZero('PED_COM_ITE','_',RECParametros.Id,3);
    ...
    ibSP.ParamByName('ped').Value  := oREPZero('PED_COM_CAB','_',RECParametros.Id,3);
...
procedure TFrmven_prc._ExecEvent(AIDLG: Word = 0);
var
  i: word;
begin
  if  (AIDLG > 0) and (AIDLG <> RECUsuarios.ID) then
  with EEvent do
  try
    UnregisterEvents;
    Events.Add(TRIM(oREPZero(REC_SHE_DEF.FB_Event,'_',RECUsuarios.ID,3)));
    Events.Add(TRIM(oREPZero(REC_SHE_DEF.FB_Event,'_',AIDLG         ,3)));
    RegisterEvents;
  except
    on E: Exception do
    begin
