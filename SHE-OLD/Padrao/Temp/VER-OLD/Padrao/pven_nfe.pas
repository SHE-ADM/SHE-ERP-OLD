...
uses
  ...
  IDGlobal, IBSQL, ActnList {Unit do componente Indy para usar Fetch() };
...
begin
  ...
  REC_SHE_DEF.FB_AlertEvent := True;
  REC_SHE_DEF.FB_Event      := Trim(AFB_Event);
...
begin
  ...
  try
    ...
    begin
      UnregisterEvents;
      Events.Add(TRIM(oREPZero(REC_SHE_DEF.FB_Event,'_',RECParametros.EP_ID,3)));
      Events.Add(TRIM(oREPZero(REC_SHE_DEF.FB_Event,'_',RECParametros.EP_ID,3) + IFThen(Pos(RECUsuarios.Grupo,'VENEXPPCPQLDCOMCRI') > 0,'_' + RECUsuarios.ID,'')));
      RegisterEvents;
    end;
  except
    on E: Exception do
...
begin
  ...
  try
      ...
      begin
        Close;
        SQL.Clear;
        SQL.Add('SELECT PK.ID FROM '  + oREPZero('NFE_CAB','_',RECParametros.EP_ID,3) + ' AS PK');
        ...
        begin
          ...
          SQL.Add('SET    FLAG = ''' + IFThen(ACTGeraNFeXML.Tag = 0,'1','0') + '''');
...
begin
  ...
  begin
    ...
    begin
      if Pos(CECDNF.Text,SBMain.Panels[3].Text) = 0 then { CHAVE }
      begin
        Editar;
...
begin
  ...
  begin
    ...
    SBMain.Panels[2].Width := Self.Width - (SBMain.Panels[0].Width + SBMain.Panels[1].Width + SBMain.Panels[3].Width + SBMain.Panels[4].Width + SBMain.Panels[5].Width) - 50;
    SBMain.Refresh;
  end;
end;
...
begin
  ...
  if Length(SBMain.Panels[3].Text) < 44 then
  ...
  { Duplicatas }
  if SBMain.Panels[4].Text <> 'D' then
  try
    DUP_001.First;
  ...
  try
    ...
    SPEdicao.ParamByName('id').Value           := 0;
    SPEdicao.ParamByName('REST').Value         := IFThen(SBMain.Panels[4].Text = 'D','D','A' );
    SPEdicao.ParamByName('CDNF').Value         := CECDNF.Value;
    ...
    SPEdicao.ParamByName('CHAV').Value         := SBMain.Panels[3].Text;
    SPEdicao.ParamByName('PROT').Value         := SBMain.Panels[4].Text;
    ...
    oCTransact(TEdicao);
    oAviso(handle,IFThen(SBMain.Panels[4].Text <> 'D','Nota fiscal transmitida com sucesso !','Nota Fiscal Denegada !'+#13+#13+
  ...
  if (SBMain.Panels[4].Text <> 'D') and (CECDRO.Value > 0) then
...
begin
  try
    ...
    SPEvent.ParamByName('AFB_EVENT_001').Value := oREPZero('CTR_PED','_',RECParametros.EP_ID,3);
    SPEvent.ParamByName('AFB_EVENT_002').Value := TRIM(oREPZero('CTR_ROM','_',RECParametros.EP_ID,3) + IFThen(Pos(RECUsuarios.Grupo,'VENEXPPCPQLDCOMCRI') > 0,'_' + RECUsuarios.ID,''));
    SPEvent.ExecProc;
...
begin
  ...
  begin
    ...
    SBMain.Panels[3].Text := EmptyStr; { CHAVE }
    SBMain.Panels[4].Text := EmptyStr; { PROTOCOLO }
    SBMain.Panels[5].Text := EmptyStr; { CSTAT }
    SBMain.Refresh;
...
begin
  ...
  try
    ...
    NFE_XML := 'C:\Sheild\NotaFiscal\NFe\arquivos\procnfe\'+oStrZero(ano,4)+oStrZero(mes,2)+'\'+SBMain.Panels[2].Text+'-procNFe.xml';

    if not FileExists(NFE_XML) then
    begin
      SBMain.Panels[1].Text := ArquivoDistribuicaoNFe(SBMain.Panels[2].Text);
      SBMain.Refresh;
...
begin
  ...
  try
    ...
    SBMain.Panels[0].Text := oNFE_API_SEFAZ;
    SBMain.Refresh;

    if LeftStr(RECParametros.NFE_API_SEFAZ,3) <> '107' then
    oException(Nil,'Falha ao tentar conectar com o sefaz !' + #13 +
                   'Motivo: ' + SBMain.Panels[0].Text       + #13 + #13 +
                   'Favor entrar em contato com o administrador do sistema.');

    SBMain.Panels[1].Text := 'Aguarde, gerando arquivo XML ...';
    SBMain.Refresh;
...
//  tIde[01] := IntToStr(Random(CECDNF.Value)+1); //<cNF> Código Numérico que compõe a chNFe de Acesso - Numero aleatorio: apenas 8 digitos: NF-e 2.00
    ...
    SBMain.Panels[3].Text := Trim(RightStr(GeraNFe(tIde,tEmit,tDest,tProd,tTotal,tTransp,tCobr,tPag,tInfAdic,autXML,True),44));
    SBMain.Refresh;
    
    if Length(SBMain.Panels[3].Text) = 44 then { Chave NFe }
    begin
      SBMain.Panels[1].Text := 'Arquivo XML gerado com sucesso';
      SBMain.Refresh;
      VALIDAR;
    end else
    begin
      SBMain.Panels[1].Text := 'Falha na geração do arquivo XML';
      SBMain.Refresh;
      Abort;
    end;
  finally
...
begin
  try
    ...
    SBMain.Panels[1].Text := 'Aguarde, assinando arquivo XML ...';
    SBMain.Panels[2].Text := EmptyStr;
    SBMain.Panels[4].Text := EmptyStr; { PROTOCOLO }
    SBMain.Panels[5].Text := EmptyStr; { CSTAT }
    SBMain.Refresh;

    if not FileExists(PChar('C:\Sheild\NotaFiscal\NFe\arquivos\assinado\NFe' + SBMain.Panels[3].Text + '-assinado.xml')) then
    begin
      SBMain.Panels[1].Text := 'Falha na assinatura da nota fiscal';
      SBMain.Panels[2].Text := 'Arquivo XML não encontrado em ..\assinado';
      SBMain.Refresh;

      Editar;
    end;

    if ValidarArquivoXML(PChar('C:\Sheild\NotaFiscal\NFe\arquivos\assinado\NFe' + SBMain.Panels[3].Text + '-assinado.xml'),'nfe_v4.00.xsd',True) = 'OK' then
    begin
      SBMain.Panels[1].Text := 'Validação efetuada com sucesso';
      SBMain.Panels[2].Text := 'Nenhum erro identificado';
      SBMain.Refresh;
    ...
    begin
      SBMain.Panels[1].Text := 'Falha na validação';
      SBMain.Panels[2].Text := 'Problemas na validação da assinatura do arquivo XML. Verifique os erros encontrados';
      SBMain.Refresh;
...
begin
  ...
  try
    ...
    SBMain.Panels[1].Text := 'Aguarde, gerando lote para envio para o sefaz ...';
    SBMain.Panels[2].Text := EmptyStr;
    SBMain.Panels[4].Text := EmptyStr; { PROTOCOLO }
    SBMain.Panels[5].Text := EmptyStr; { CSTAT }
    SBMain.Refresh;
    ...
    if x = 1 then
    begin
      SBMain.Panels[1].Text := 'Lote gerado com sucesso ! Aguardando autorização do sefaz ...';
      SBMain.Refresh;
      ...
      repeat
        ...
        if LeftStr(SBMain.Panels[1].Text,3) = '103' then { Lote recebido com sucesso }
        begin
          SBMain.Panels[4].Text := Trim(RightStr(SBMain.Panels[1].Text,Length(SBMain.Panels[1].Text) - Pos('#',SBMain.Panels[1].Text)));
          SBMain.Refresh;
          Break;
        end;
    ...
    if x = -1 then
    SBMain.Panels[1].Text := 'Falha na geração do lote' else

    if x = 0 then
    SBMain.Panels[1].Text := 'Operação Cancelada' else
    ...
    if SBMain.Panels[4].Text = EmptyStr then { PROTOCOLO }
    begin
      EDITAR;
...
begin
  ...
  try
    ...
    SBMain.Panels[1].Text := 'Aguardando resposta do sefaz ...';
    SBMain.Panels[2].Text := 'Autorização em andamento';
    SBMain.Panels[5].Text := EmptyStr; { CSTAT }
    SBMain.Refresh;

    repeat
      inc(y);
      SBMain.Panels[1].Text := NfeRetAutorizacao(SBMain.Panels[4].Text);
      SBMain.Refresh;

      if Pos('AUTORIZADO',UPPERCASE(SBMain.Panels[1].Text)) > 0 then
      begin
        SBMain.Panels[1].Text := 'Autorizado uso de NFe';
        SBMain.Panels[2].Text := EmptyStr;
        SBMain.Refresh;
      ...
      if (Pos(LeftStr(SBMain.Panels[1].Text,3),'100105110135') = 0) then
      begin
        if Pos('REJ',UPPERCASE(SBMain.Panels[1].Text)) > 0 then
        begin
          SBMain.Panels[2].Text := Trim(Copy(SBMain.Panels[1].Text  ,Pos('REJ',UPPERCASE(SBMain.Panels[1].Text)),Length(SBMain.Panels[1].Text)));
          SBMain.Panels[1].Text := 'Autorização Negada';
          SBMain.Refresh;
        end else

        if Pos('DUP',UPPERCASE(SBMain.Panels[2].Text)) > 0 then
        begin
          SBMain.Panels[2].Text := Trim(Copy(SBMain.Panels[1].Text  ,Pos('DUP',UPPERCASE(SBMain.Panels[1].Text)),Length(SBMain.Panels[1].Text)));
          SBMain.Panels[1].Text := 'Autorização Negada';
          SBMain.Refresh;

          if Pos('CHNFE',UPPERCASE(SBMain.Panels[2].Text)) > 0 then
          begin
            if Length(Copy(SBMain.Panels[2].Text,Pos('CHNFE',UPPERCASE(SBMain.Panels[2].Text)) + 6,44)) = 44 then
      ...
      SBMain.Panels[2].Text := 'Tentativa(s): ' + INTTOSTR(Y);
      SBMain.Refresh;
    ...
    if SBMain.Panels[2].Text <> EmptyStr then
    begin
      EDITAR;
      oException(Nil,SBMain.Panels[2].Text + #13 +
...
begin
  SBMain.Panels[5].Text := EmptyStr;
  SBMain.Refresh;

  ACTGeraLote.Execute;
  ACTConsultaXML.Execute;

  if (Pos(LeftStr(SBMain.Panels[1].Text,3),'100110135') = 0) then
  try
    FrmNFeConsultaSefaz := TFrmNFeConsultaSefaz.Create(Self,'Transmitindo NFe ...',SBMain.Panels[3].Text);
    FrmNFeConsultaSefaz.EMConsulta.Lines.Add('');
    FrmNFeConsultaSefaz.EMConsulta.Lines.Add(SBMain.Panels[2].Text);
    FrmNFeConsultaSefaz.ShowModal;
  finally
    if SBMain.Panels[2].Text = EmptyStr then
    SBMain.Panels[5].Text := FrmNFeConsultaSefaz.EDcStat.Text;
    SBMain.Refresh;

    FreeAndNil(FrmNFeConsultaSefaz);
  end;

  if Pos(SBMain.Panels[5].Text,'100110') = 0 then
  begin
    Editar;
  ...
  try
    SBMain.Panels[1].Text := 'Aguarde, registrando NFe ...';
    SBMain.Refresh;

    SALVA_NF;

    SBMain.Panels[1].Text := 'Pronto';
    SBMain.Panels[2].Text := 'NFe registrada com sucesso';
    SBMain.Refresh;
  except
    SBMain.Panels[1].Text := 'Falha no registro da NFe';
    SBMain.Refresh;
  end;

  try
    SBMain.Panels[1].Text := 'Aguarde, gerando danfe ...';
    SBMain.Refresh;

    GERA_DANFE;

    SBMain.Panels[1].Text := 'Pronto';
    SBMain.Panels[2].Text := 'Danfe gerada com sucesso';
    SBMain.Refresh;
  except
    SBMain.Panels[1].Text := 'Falha na geração da danfe';
    SBMain.Refresh;
  end;
...
begin
  ...
  begin
    PCEdicao.ActivePage := TSProduto;
    oException(DBGNFE,'Itens da nota fiscal não Informados !');
  end;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        begin
          DBGNFE.FocusedAbsoluteIndex := DBGNFENFE_QCOM.Index;
          ACTCheckConstraints.Hint    := 'Quantidade não Informada !';
        end;

        if NFE_001NFE_VUNCOM.AsFloat = 0 then
        begin
          DBGNFE.FocusedAbsoluteIndex := DBGNFENFE_VUNCOM.Index;
        ...
        begin
          PCEdicao.ActivePage := TSProduto;
          oException(DBGNFE,ACTCheckConstraints.Hint + #13 +
