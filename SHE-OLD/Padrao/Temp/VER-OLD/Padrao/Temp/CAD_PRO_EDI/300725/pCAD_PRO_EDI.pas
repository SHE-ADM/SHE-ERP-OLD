...
type
  TFrmCAD_PRO_EDI = class(TFrmPadr3)
    ...
    MKP_MPG_ID: TdxImageEdit;
    LAMKP_MPG_ID: TLabel;
    CEMKP_MSP_ID: TdxImageEdit;
    LAMKP_MSP_ID: TLabel;
    GroupBox2: TGroupBox;
    LAMKP_BLG_ID: TLabel;
    LAMKP_TRY_ID: TLabel;
...
begin
  ...
  CAD_PRO_GRDVPRC_PRZ.Value := CEVPRC_PAD_PRZ.Value;
  CAD_PRO_GRDVPRC_PRO.Value := CEVPRC_PAD_PRO.Value;

  CAD_PRO_GRDSCT_ID.Value := StrToInt(IEIDSCT.Text);
  CAD_PRO_GRDSCT_NO.Value := IEIDSCT.Descriptions[IEIDSCT.Values.IndexOf(IEIDSCT.Text)];

  CAD_PRO_GRDMKP_ID.Value := StrToInt(IEMKP_IDML.Text);
  CAD_PRO_GRDMKP_NO.Value := IEMKP_IDML.Descriptions[IEMKP_IDML.Values.IndexOf(IEMKP_IDML.Text)];
...
begin
  ...
  try
    ...
    SPEdicao.ParamByName('AATJ_VPRC_PAD').Value := ATJ_VPRC_PAD.Value;
    SPEdicao.ParamByName('AATJ_VPRC_PRZ').Value := ATJ_VPRC_PRZ.Value;
    SPEdicao.ParamByName('AATJ_VPRC_PRO').Value := ATJ_VPRC_PRO.Value;
    SPEdicao.ParamByName('AATJ_VPRC_MKP').Value := ATJ_VPRC_MKP.Value;
    SPEdicao.ParamByName('AATJ_VPRC_DSC').Value := ATJ_VPRC_DSC.Value;

    SPEdicao.ParamByName('ALJV_VPRC_PAD').Value := LJV_VPRC_PAD.Value;
    SPEdicao.ParamByName('ALJV_VPRC_PRZ').Value := LJV_VPRC_PRZ.Value;
    SPEdicao.ParamByName('ALJV_VPRC_PRO').Value := LJV_VPRC_PRO.Value;
    SPEdicao.ParamByName('ALJV_VPRC_MKP').Value := LJV_VPRC_MKP.Value;
    SPEdicao.ParamByName('ALJV_VPRC_DSC').Value := LJV_VPRC_DSC.Value;

    SPEdicao.ParamByName('AVAR_VPRC_PAD').Value := VAR_VPRC_PAD.Value;
    SPEdicao.ParamByName('AVAR_VPRC_PRZ').Value := VAR_VPRC_PRZ.Value;
    SPEdicao.ParamByName('AVAR_VPRC_PRO').Value := VAR_VPRC_PRO.Value;
    SPEdicao.ParamByName('AVAR_VPRC_MKP').Value := VAR_VPRC_MKP.Value;
    SPEdicao.ParamByName('AVAR_VPRC_DSC').Value := VAR_VPRC_DSC.Value;

    SPEdicao.ParamByName('AREP_VPRC_PAD').Value := REP_VPRC_PAD.Value;
    SPEdicao.ParamByName('AREP_VPRC_PRZ').Value := REP_VPRC_PRZ.Value;
    SPEdicao.ParamByName('AREP_VPRC_PRO').Value := REP_VPRC_PRO.Value;
    SPEdicao.ParamByName('AREP_VPRC_MKP').Value := REP_VPRC_MKP.Value;
    SPEdicao.ParamByName('AREP_VPRC_DSC').Value := REP_VPRC_DSC.Value;
    ...
    SPEdicao.ParamByName('AFIN_CAD_ID').Value :=
    SPEdicao.ParamByName('AFIN_EST_ID').Value :=

    SPEdicao.ParamByName('AORIG' ).Value :=
    SPEdicao.ParamByName('ACPAIS').Value :=

    SPEdicao.ParamByName('AINFADCAD').Value :=
    SPEdicao.ParamByName('AINFADTEC').Value :=
...
//      CEUQTDE_EST_MIN_LV.Value := Current.ByName('UQTDE_EST_MIN_SITE').AsCurrency;
        ...
        IEIDCOL.Text  := IntToStr(Current.ByName('COL_ID').AsInteger);
