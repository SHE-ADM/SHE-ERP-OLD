...
begin
  siVIS.Enabled := false;
  siALT.Enabled := bPSQUSER('ALTERA','Estoque','Etiquetas','Permissões Gerais');
  siCAN.Enabled := bPSQUSER('EXCLUI','Estoque','Etiquetas','Permissões Gerais');
  siCSE.Enabled := bPSQUSER('EXCLUI','Estoque','Expedição','Permissões Gerais');
...
begin
  ...
  begin
    ...
    SQL.Add('FROM ' + oREPZero('ROM_ITE','_',RECParametros.Id,3) + ' AS FK');
    SQL.Add('JOIN ' + oREPZero('ROM_CAB','_',RECParametros.Id,3) + ' AS PK ON (PK.IDPK = FK.IDPK)');
...
begin
  ...
  try
    ...
    begin
      ...
      SQL.Add('JOIN ' + oREPZero('PED_VEN_ITE','_',RECParametros.Id,3) + ' AS PK ON (PK.IDPK = FK.IDPK AND PK.IDCP = FK.IDCP AND PK.VPRC_COM = FK.VPRC_COM)');
      ExecQuery;
    end;
    ...
    begin
      ...
      begin
        Close;
        SQL.Clear;
        SQL.Add('UPDATE ' + oREPZero('PED_VEN_ITE','_',RECParametros.Id,3) + ' AS PK');
...
begin
  ...
  try
    ...
    begin
      SQL.Clear;
      SQL.Add('SELECT PK.ID,ROM_DERO,ROM_DROM,CLI_FANT,LOGIN,REP_FANT,ROM_QTVE,ROM_RLVE,PK.ROM_CDNF,PK.ROM_DNFS');
      ...
      SQL.Add('JOIN TAB_USER AS CV ON (CV.IDCV = PK.IDCV)');

      SQL.Add('WHERE PK.IDPK = '''+inttostr(lanped.Tag)+'''');
      SQL.Add('AND   FK.IDCP = '''+inttostr(lacpro.Tag)+'''');
      Open;
...
procedure Tfrmeti_pro.ACTExecEventExecute(Sender: TObject);
begin
  inherited;
  oSP_CAD_PRO_EST_LAN(SQLEvent,RECParametros.ID,IntToStr(lacpro.Tag));
end;
