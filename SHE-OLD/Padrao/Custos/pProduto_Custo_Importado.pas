...
uses pProduto_Custo_Importado_Edicao, pRelatorios_OLD;
...
begin
  ...
  { ACCESS MANAGER }
  REC_SHE_DEF.FB_Event := 'Custos Importados'; { Eventos }

  { GRANT USER }
  REC_SHE_DEF.GDescricao  := 'Custos';
  REC_SHE_DEF.GReferencia := 'Produtos';
  REC_SHE_DEF.GRegra      := 'Importados';
...
procedure TFrmProduto_Custo_Importado.ACTRelatoriosExecute(
  Sender: TObject);
begin
  FrmRelatorios_OLD := TFrmRelatorios_OLD.Create(Nil,'CUSTO DE PRODUTOS IMPORTADOS',18);
