...
type
  Tfrmrec_pag_car = class(Tfrmpadr3)
  ...
  private
    ...
    procedure PESQUISA_CONTA(BRet: boolean;txt,cField: string);
...
var
  frmrec_pag_car: Tfrmrec_pag_car;
  Ano, Mes, Dia: Word;
  demi: TDate;
  chav,cdnf: String;
...
uses ppesquisa, uPrincipal, qcob_ped,
  qcob_rom;

{$R *.dfm}

procedure Tfrmrec_pag_car.ALTERA_STFI(tipo,doct: string);
begin
  with fin_rec_rec do
  begin
    tag := 0;
    SQL.Clear;
    SQL.Add('SELECT   FIN_REC_CAR_REC.*,CLI_FANT,CLI_RAZA,USU_DUSU,REP_FANT');
    if tipo = 'CARTEIRA' then
    SQL.Add('FROM   '+SLPrincipal.Values['fin_rec_car_bai']+' "FIN_REC_CAR_REC"') else
    SQL.Add('FROM   '+SLPrincipal.Values['fin_rec_car_rec']+' "FIN_REC_CAR_REC"');
    SQL.Add('LEFT     OUTER JOIN CAD_CLI ON FIN_REC_CAR_REC.FIN_CCLI = CAD_CLI.ID');
    SQL.Add('LEFT     OUTER JOIN CAD_REP ON FIN_REC_CAR_REC.FIN_CREP = CAD_REP.ID');
    SQL.Add('LEFT     OUTER JOIN CAD_USU ON FIN_REC_CAR_REC.FIN_CVEN = CAD_USU.USU_CUSU');
...
procedure Tfrmrec_pag_car.INCLUI_CONTA;
var
  BRet: boolean;
begin
  try
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          cadastroFIN_DERD.Value := fin_rec_baiFIN_DERD.AsString;
          cadastro.Post;
        end;
...
procedure Tfrmrec_pag_car.PESQUISA_CONTA(BRet: boolean;txt,cField: string);
begin
  with fin_rec_bai do
  begin
    SQL.Clear;
    SQL.Add('SELECT   FIN_REC_CAR_BAI.*,CLI_FANT,CLI_RAZA,USU_DUSU,REP_FANT');
    SQL.Add('FROM   '+SLPrincipal.Values['fin_rec_car_bai']+' "FIN_REC_CAR_BAI"');
    SQL.Add('LEFT     OUTER JOIN CAD_CLI ON FIN_REC_CAR_BAI.FIN_CCLI = CAD_CLI.ID');
    SQL.Add('LEFT     OUTER JOIN CAD_REP ON FIN_REC_CAR_BAI.FIN_CREP = CAD_REP.ID');
    SQL.Add('LEFT     OUTER JOIN CAD_USU ON FIN_REC_CAR_BAI.FIN_CVEN = CAD_USU.USU_CUSU');
    SQL.Add('WHERE    FIN_REC_CAR_BAI.ID > 0');

    if BRet then
    SQL.Add('AND     '+cField+' = '''+txt+'''') else
    SQL.Add('AND     '+cField+' LIKE '''+txt+'%''');
    SQL.Add('ORDER BY FIN_REC_CAR_BAI.ID DESC');
    Open;
  end;
end;

procedure Tfrmrec_pag_car.AjustaForm;
begin
...
procedure Tfrmrec_pag_car.ABRE_TABELA;
begin
  oOTransact(TSheild);
  with consulta_s do
  begin
...
procedure Tfrmrec_pag_car.siPSQClick(Sender: TObject);
begin
  ...
  frmpesquisa.cbCAMPO.Enabled := true;
  ...
  try
    ...
    begin
      ...
      begin
        begin
          PESQUISA_CONTA(false,edtxt.Text,cField);
          fin_rec_bai.Last;
          ...
          begin
            ...
            begin
              if fin_rec_bai.RecordCount = 1 then
              begin
...
procedure Tfrmrec_pag_car.FormCreate(Sender: TObject);
begin
  inherited;
  ...
  begin
    SQL.Clear;
    SQL.Add('SELECT   FIN_DOCT,FIN_VALO,FIN_STFI FROM '+SLPrincipal.Values['fin_rec_car_bai']);
...
procedure Tfrmrec_pag_car.cadastroAfterCancel(DataSet: TDataSet);
begin
  if tag = 0 then
  begin
    finA.Enabled := true;
...
procedure Tfrmrec_pag_car.cadastroAfterDelete(DataSet: TDataSet);
begin
  tSHEILD.CommitRetaining;
end;

procedure Tfrmrec_pag_car.cadastroAfterEdit(DataSet: TDataSet);
begin
  if tag = 0 then
  begin
    finA.Enabled := false;
...
procedure Tfrmrec_pag_car.cadastroAfterOpen(DataSet: TDataSet);
begin
  if tag = 0 then
  begin
    finA.Enabled := true;
...
procedure Tfrmrec_pag_car.cadastroBeforeCancel(DataSet: TDataSet);
begin
  if tag = 0 then
  begin
    finA.Enabled := true;
...
procedure Tfrmrec_pag_car.cadastroAfterPost(DataSet: TDataSet);
var
  wRec: TBookMark;
begin
  tSHEILD.CommitRetaining;
...
procedure Tfrmrec_pag_car.finAClick(Sender: TObject);
begin
  cadastro.Edit;
end;

procedure Tfrmrec_pag_car.finDClick(Sender: TObject);
begin
  if not cadastro.Fields[0].IsNull then
  cadastro.Delete;
end;

procedure Tfrmrec_pag_car.siSAVClick(Sender: TObject);
begin
  cadastro.Post;
end;

procedure Tfrmrec_pag_car.finCClick(Sender: TObject);
begin
  cadastro.Cancel;
end;

procedure Tfrmrec_pag_car.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  case key of
...
procedure Tfrmrec_pag_car.dbgConsultaKeyDown(Sender: TObject;
...
procedure Tfrmrec_pag_car.siVALClick(Sender: TObject);
var
  cdbx: integer;
  i: word;
begin
  ...
  if oYesNo(handle,'Validar recebimento da carteira ?') = mrno then
  abort;
  ...
  raise exception.Create('Parcela sem marcação para recebimento em carteira');
...
procedure Tfrmrec_pag_car.dbgConsultaCustomDrawCell(Sender: TObject;
...
procedure Tfrmrec_pag_car.cadastroBeforePost(DataSet: TDataSet);
begin
...
procedure Tfrmrec_pag_car.siCANClick(Sender: TObject);
begin
  if oYesNo(handle,'Confirma cancelamento e exclusão das carteiras selecionadas ?') = mrno then
     abort;
  ABRE_TABELA;
end;

procedure Tfrmrec_pag_car.cadastroFIN_STFIValidate(Sender: TField);
begin
  if tag = 0 then
  begin
...
procedure Tfrmrec_pag_car.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  inherited;
  if frmrec_pag_car <> nil then
  begin
    frmrec_pag_car.Release;
    frmrec_pag_car := nil;
  end;
end;

procedure Tfrmrec_pag_car.FormCloseQuery(Sender: TObject;
...
procedure Tfrmrec_pag_car.siRECClick(Sender: TObject);
begin
 if cadastro.State in [dsEdit,dsInsert] then
 cadastro.Post;
...
procedure Tfrmrec_pag_car.siCREClick(Sender: TObject);
begin
  if cadastro.State in [dsEdit,dsInsert] then
  cadastro.Post;
...
procedure Tfrmrec_pag_car.cadastroBeforeDelete(DataSet: TDataSet);
begin
...
procedure Tfrmrec_pag_car.lan_chqAfterPost(DataSet: TDataSet);
var
  wRec: TBookMark;
begin
  tSHEILD.CommitRetaining;
...
procedure Tfrmrec_pag_car.cadastroFIN_PMULValidate(Sender: TField);
begin
  if tag = 0 then
  begin
...
procedure Tfrmrec_pag_car.cadastroFIN_PDESValidate(Sender: TField);
begin
  if tag = 0 then
  cadastroFIN_VDES.Value := (cadastroFIN_VALO.AsFloat * cadastroFIN_PDES.AsFloat) / 100;
end;

procedure Tfrmrec_pag_car.cadastroFIN_VPAGValidate(Sender: TField);
begin
  if tag = 0 then
  begin
...
procedure Tfrmrec_pag_car.dbgConsultaFIN_STFIValidate(Sender: TObject;
  var ErrorText: String; var Accept: Boolean);
begin
  ...
  begin
    ...
    begin
      ...
      ALTERA_STFI('CARTEIRA',cadastroFIN_DOCT.AsString);
    end;  
...
procedure Tfrmrec_pag_car.siVINClick(Sender: TObject);
begin
  PESQUISA_CONTA(true,fin_recFIN_DOCT.AsString,'FIN_DOCT');
  if fin_rec_bai.Fields[0].IsNull then
  raise exception.Create('Documento não encontrado !');
...
procedure Tfrmrec_pag_car.tsENCShow(Sender: TObject);
begin
  dbgencargos.SetFocus;
  dbgencargos.FocusedColumn := 2;
end;

procedure Tfrmrec_pag_car.siVITDClick(Sender: TObject);
begin
  fin_rec.First;
...
procedure Tfrmrec_pag_car.siTPGClick(Sender: TObject);
var
  NewString: string;
  ClickedOK: Boolean;
begin
...
procedure Tfrmrec_pag_car.SpeedItem1Click(Sender: TObject);
begin
  if not frmprincipal.ACESSO(frmprincipal.cad_usuUSU_CUSU.AsString,'USU_AUTO','Financeiro','Contas a Receber','Status de Recebimentos',false) then
  Databaseerror('ACESSO NEGADO !'+#13+'Contate o admnistrador do sistema.');
...
procedure Tfrmrec_pag_car.lan_chqAfterDelete(DataSet: TDataSet);
begin
  tSHEILD.CommitRetaining;
end;

procedure Tfrmrec_pag_car.dbgConsultaFIN_DPAGValidate(Sender: TObject;
  var ErrorText: String; var Accept: Boolean);
begin
  if not frmprincipal.ACESSO(frmprincipal.cad_usuUSU_CUSU.AsString,'USU_AUTO','Financeiro','Contas a Receber','Status de Recebimentos',false) then
  Databaseerror('ACESSO NEGADO !'+#13+'Contate o admnistrador do sistema.');
...
procedure Tfrmrec_pag_car.dbgrec_recKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  case key of
