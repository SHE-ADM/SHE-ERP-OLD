...
begin
  if Self <> Nil then
     try
       ...
       { Altura }
       if PCPrincipal.Align = alTOP then

       if Screen.Height > 900 then
       PCPrincipal.Height := 320 else { 10 itens }

       if Self.Height = 900 then
       PCPrincipal.Height := 235 else { 7 itens }

       if Self.Height > 800 then { 25% }
       PCPrincipal.Height := 250 else { 8 itens }

       if Self.Height = 800 then { 25% }
       PCPrincipal.Height := 235 else { 7 itens }
       PCPrincipal.Height := 215;     { 6 itens }
...
procedure TFrmSHE_DEF_EDI.DBGConsultaCustomDrawCell(Sender: TObject;
...
procedure TFrmSHE_DEF_EDI.DBGConsultaKeyDown(Sender: TObject;
...
procedure TFrmSHE_DEF_EDI.ACTEAppendExecute(Sender: TObject);
begin
...
procedure TFrmSHE_DEF_EDI.ACTEEditExecute(Sender: TObject);
begin
...
procedure TFrmSHE_DEF_EDI.ACTEDeleteExecute(Sender: TObject);
begin
...
procedure TFrmSHE_DEF_EDI.ACTEPostExecute(Sender: TObject);
begin
...
procedure TFrmSHE_DEF_EDI.ACTECancelExecute(Sender: TObject);
begin
...
procedure TFrmSHE_DEF_EDI.ACTPSQ_OKExecute(Sender: TObject);
begin
...
procedure TFrmSHE_DEF_EDI.ACTPSQ_FOCUSExecute(Sender: TObject);
begin
  if (ACTPesquisa.Enabled) and (BEPSQ_CAD.Enabled) then
  BEPSQ_CAD.SetFocus(False);
end;

procedure TFrmSHE_DEF_EDI.BEPSQ_CADKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrmSHE_DEF_EDI.BEPSQ_CADCurChange(Sender: TObject);
begin
  BEPSQ_CAD.Text := BEPSQ_CAD.CurText;
end;

procedure TFrmSHE_DEF_EDI.BDPSQ_PER_INICurChange(Sender: TObject);
begin
  BDPSQ_PER_INI.Date := BDPSQ_PER_INI.CurDate;
end;

procedure TFrmSHE_DEF_EDI.BDPSQ_PER_FIMCurChange(Sender: TObject);
begin
