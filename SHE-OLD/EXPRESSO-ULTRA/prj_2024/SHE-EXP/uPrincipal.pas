...
uses bPrincipal,
  ...
  pEXP_SEP_MAN, pctr_ped, pctr_rom, pEtiqueta_Geral, pNFeConsulta,
  pNFeSaida, pNFeSaidaConsulta;
...
begin
  bPSQUSER('VISUALIZA','Nota Fiscal','Vendas','Incluir',True);
  uFrmCreate(Nil,TFrmNFeConsulta,FrmNFeConsulta);
end;

procedure TFrmPrincipal.ACTNFE_SAI_LANExecute(Sender: TObject);
begin
  if bPSQUSER('INCLUI','Logística','Saídas','Permissões Gerais') then
     TFrmNFeSaida.ExecForm(Nil,FrmNFeSaida) else
  begin
    bPSQUSER('VISUALIZA','Logística','Saídas','Permissões Gerais',True);
