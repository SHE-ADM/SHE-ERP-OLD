...
procedure TqrpProduto_Venda.WinControlFormCreate(Sender: TObject);
          ...
          begin
            ...
            { Definindo Tabelas }
            RECRElatorio.TAB_PK     := oREPZero(IFThen(RECRelatorio.Tipo = 'Programações','PED_PRG_CAB','PED_VEN_CAB'),'_',RECParametros.Id,3);
            RECRElatorio.TAB_PK_ITE := oREPZero(IFThen(RECRelatorio.Tipo = 'Programações','PED_PRG_ITE','PED_VEN_ITE'),'_',RECParametros.Id,3);
            if ((RECRelatorio.Status = 'Faturados') and (RECRelatorio.Tipo <> 'Programações')) then
            begin
              RECRElatorio.TAB_FK     := oREPZero('ROM_CAB','_',RECParametros.Id,3);
              RECRElatorio.TAB_FK_ITE := oREPZero('ROM_ITE','_',RECParametros.Id,3);
            end;
