...
begin
  ...
  begin
    ...
    SQL.Add('       FR.API_ST,FR.API_CA,FR.API_DTCA,FR.FIN_CDBC,');
    SQL.Add('       FR.FIN_OBSE');
    SQL.Add('FROM '+RECPrincipal.Tabela+' AS FR');
    SQL.Add('JOIN   CAD_CLI AS CL ON (CL.ID = FR.FIN_CCLI)');
    SQL.Add('WHERE  FR.IDEP = '''+RECParametros.Id +'''');
    SQL.Add('AND    FR.ID   = '''+RECPrincipal.IDPK+'''');
    ExecQuery;
    ...
    CEVendedor.Value      := Current.ByName('FIN_CVEN').AsInteger;
    IEVendedor.Text       := Current.ByName('FIN_CVEN').AsString;
    CERepresentante.Value := Current.ByName('FIN_CREP').AsInteger;
    ...
    begin
      ...
      ACDBX := '0';
      ASTDO := IFThen(Pos('BAN',UpperCase(RECPrincipal.Tabela)) > 0,'B 1','C 1');

      DEFatura.Date := RECParametros.DTServer;
      DEFatura.Hint := FormatDateTime('dd/mm/yyyy',RECParametros.DTServer);
