...
uses uPrincipal,  ppesquisa, pven_nfe;
...
begin
  ...
  dbgConsultaROM_TOTA.Visible   := (frmprincipal.ACESSO(RECUsuarios.ID,'USU_AUTO','Vendas','Romaneios','Visualizar Total Faturado',false));
  dbgconsulta.ShowSummaryFooter := (frmprincipal.ACESSO(RECUsuarios.ID,'USU_AUTO','Vendas','Romaneios','Totalizadores'            ,false));
  ...
  begin
    ...
    SelectSQL.Add('AND      ROM_CAB.ROM_CVEN = '''+RECUsuarios.ID+'''');
...
begin
  if CadastroID.AsInteger = 0 then
     oException(DBGConsulta,'Romaneio não Informado !');
  ...
  begin
    Close;
    SQL.Clear;
    SQL.Add('SELECT PK.IDPK,CAST(PK.DTCA AS DATE),PK.DEPK FROM ' + oREPZero('PED_VEN_CAB','_',RECParametros.Id,3) + ' AS PK');
    SQL.Add('WHERE  PK.CDRO = ''' + CadastroID.AsString + '''');
    ExecQuery;
  end;
  ...
  try
   {rom_ite.First;
    ...
    begin
      ...
      SPEdicao.ParamByName('dcad').Value := SQLConsulta.Current.Vars[1].AsDateTime;
      SPEdicao.ParamByName('docu').Value := SQLConsulta.Current.Vars[2].AsString;
      SPEdicao.ParamByName('flag').Value := 'R';
      SPEdicao.ParamByName('cdet').Value := '';
      SPEdicao.ParamByName('dsep').Value := '';
    ...
    begin
      ...
      SQL.Add('WHERE IDPK     = '''+SQLConsulta.Current.Vars[0].AsString+'''');
      ExecQuery;

      Close;
      SQL.Clear;
      SQL.Add('DELETE FROM '   + oREPZero('ROM_CAB','_',RECParametros.Id,3));
      SQL.Add('WHERE  ID = ''' + CadastroID.AsString + '''');
      ExecQuery;

      Close;
      SQL.Clear;
      SQL.Add('DELETE FROM '         + oREPZero('ROM_ITE','_',RECParametros.Id,3));
      SQL.Add('WHERE  ROM_CCAB = ''' + CadastroID.AsString + '''');
      ExecQuery;
    end;

    oCTransact(TEdicao);
    oAviso(handle,'Romaneio cancelado com sucesso !');
  except
    on E: Exception do
    begin
      oCTransact(TEdicao,ltRollback);
      oException(DBGConsulta,'Falha ao tentar cancelar romaneio !' +#13+
...
begin
  ...
  try
    ...
    begin
      ...
      frmven_nfe.nfe_001NFE_VUNCOM.AsFloat  := SQLConsulta.Current.ByName('VUNCOM').AsFloat;
      frmven_nfe.nfe_001NFE_PREC.AsFloat    := SQLConsulta.Current.ByName('VUNCOM').AsFloat;
...
procedure Tfrmctr_rom.FormResize(Sender: TObject);
begin
  RECDefault.MainArea := True;
  inherited;
end;

procedure Tfrmctr_rom.CadastroBeforeCancel(DataSet: TDataSet);
begin
  {}
end;
