...
type
  Tfrmven_ped = class(TForm)
    ...
    DBGPED_HSTDTCA: TdxDBGridDateColumn;
    ...
    procedure DBGEdicaoKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure EDDEPKKeyPress(Sender: TObject; var Key: Char);
    ...
    procedure EDDEPKChange(Sender: TObject);
    procedure DTSPED_HSTDataChange(Sender: TObject; Field: TField);
    procedure CEVFRTChange(Sender: TObject);
...
begin
  ...
  DBGEdicaoVPRC_COM.DisableEditor := NOT (frmprincipal.ACESSO(RECUsuarios.ID,'USU_AUTO','Vendas','Pedidos','Alterar Preço da Tabela',false));

{  EDDEPK.ReadOnly := NOT (frmprincipal.ACESSO(RECUsuarios.ID,'USU_AUTO','Vendas','Pedidos','Alterar Número',False));
  if EDDEPK.ReadOnly then
  EDDEPK.Cursor := crNo else
  EDDEPK.Cursor := crDefault;
}

  if RECUsuarios.Grupo = 'VEN' then
  begin
    edcven.Text := RECUsuarios.ID;
...
begin
  try
    ...
    try
      ...
      begin
        ...
        EDDEPK.Text := frmctr_ped.cadastroDEPK.AsString;
  ...
  if (EDDEPK.Text <> EmptyStr) and (EDDEPK.Text <> '0') then
  DBGEdicao.SetFocus else
  EDDCLI.SetFocus;

  EDDROM.ReadOnly := True;
  _PED_HST;
end;
...
begin
  ...
  begin
    ...
    begin
      Self.Font.Size := 8;
      DBGEdicaoDGCP.Font.Size := 8;
      DBGEdicaoDECP.Font.Size := 8;
    end;

    if Screen.Width = 1024 then
    begin
      DBGEdicaoDGCP.Width := 100;
      DBGEdicaoDECP.Width := 200;
    end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          EDDEPK.Text := RETORNA_PEDIDO(frmimporta_geral.imp_cabROM_DERO.AsString);
          if EDDEPK.Text = frmimporta_geral.imp_cabROM_DERO.AsString then
          EDDEPK.Text := '0';
        end;
...
procedure Tfrmven_ped.EDDEPKChange(Sender: TObject);
begin
  SISalva.Enabled := (TAG = 0);

  if TAG = 0 then
  if EDDEPK.Text = EmptyStr then
     EDDEPK.Font.Style := [] else
     EDDEPK.Font.Style := [fsBold];
end;

procedure Tfrmven_ped.CEVFRTChange(Sender: TObject);
begin
  SISalva.Enabled := (TAG = 0);

  if CEVFRT.Value > 0 then
     CEVFRT.Font.Style := [fsBold] else
     CEVFRT.Font.Style := [];
end;
...
begin
  ...
  if (EDDEPK.Text = '0') or (EDDEPK.Text = '') then
      EDDEPK.Text := edcdro.Text;
  ...
  try
    try
     ...
     // ibSP.ParamByName('derd').Value := edcdrd.Hint;
      ibSP.ParamByName('dero').Value := EDDEPK.Text;
    ...
    begin
      try
        ...
        begin
          ...
          ibSP.ParamByName('docu').Value := trim(copy(EDDEPK.Text,1,10));
    ...
    frmprincipal.Log_Eve('Vendas',LOWERCASE(CBSTPD.Text),edcdro.Hint,EDDEPK.Text,EDDEPK.Text,LOWERCASE(eddcli.Text),AUTORIZACAO,TP_AUTORIZACAO);
...
begin
  ...
  if psq_cli.RecNo = 0 then
     oException(CEIDCD,'Cliente não Cadastrado');
  ...
  begin
    ...
    { Consulta última movimentação }
    with SQLConsulta do
    begin
      Close;
      SQL.Clear;
      SQL.Add('SELECT FIRST  1');
      SQL.Add('       PK.IDPK,PK.IDCV,CV.LOGIN AS DECV,PK.IDCR,CR.FANTASIA AS DECR,PK.IDCT,CT.FANTASIA AS DECT,PK.MFRT,');
      SQL.Add('       PK.CDPD,PK.STPD,TB_PD.PED_FAT AS FAPD,TB_PD.PED_EST  AS BEPD,');
      SQL.Add('       PK.CDCO,PK.STCO,PK.CDPG,TB_PG.DESCRICAO AS DEPG');
      ...
      SQL.Add('JOIN TAB_TPO  AS TB_PD ON (TB_PD.ID = PK.CDPD)');
      ...
      begin
        { Vendedor }
        EDCVEN.Text := Current.ByName('IDCV').AsString;
        CBDVEN.Text := Current.ByName('DECV').AsString;

        { Representante }
        EDCREP.Text := Current.ByName('IDCR').AsString;
        CBDREP.Text := Current.ByName('DECR').AsString;

        { Frete }
        CBDTRA.Tag  := Current.ByName('IDCT').AsInteger;
        CBDTRA.Text := Current.ByName('DECT').AsString;


        { Tipos }
        CBSTPD.Text := Current.ByName('STPD').AsString;
        EDSTPD.Text := Current.ByName('STPD').AsString;
        EDFATU.Text := Current.ByName('FAPD').AsString;
        EDBEST.Text := Current.ByName('BEPD').AsString;

        { Cobrança }
        CBSTCO.Text := Current.ByName('STCO').AsString;
        EDCPAG.Text := Current.ByName('CDPG').AsString;
        CBDPAG.Text := Current.ByName('DEPG').AsString;
...
procedure Tfrmven_ped.EDDEPKKeyPress(Sender: TObject; var Key: Char);
begin
  key := #0;
end;
...
procedure Tfrmven_ped.edpdscValidate(Sender: TObject;
  var ErrorText: String; var Accept: Boolean);
begin
  if not frmprincipal.ACESSO(RECUsuarios.ID,'USU_AUTO','Vendas','Pedidos','Aplicar Desconto Normal',False) then
  begin
...
begin
 cpro := DBGEdicao.EditingText;
  ...
  begin
    if not frmprincipal.ACESSO(RECUsuarios.ID,'USU_AUTO','Vendas','Pedidos','Alterar Descrição',False) then
    begin
...
begin
  ...
  begin
    ...
    SQL.Add('PK.IDPK,PK.DEPK ,CAST(PK.DTCA AS DATE)  AS DTPK,CAST(PK.DTCA AS TIME) AS HTPK,PK.DTCA,');
    SQL.Add('PK.STPD,PK.STCO,PK.RECO,TB_PG.DESCRICAO AS DEPG,PK.DEST,');
    ...
    SQL.Add('CAST(FK.VPRC_COM AS NUMERIC(5,2)) AS VPRC_COM,FK.PDSC,FK.TCDE');
...
begin
  if TAG = 0 then
  if (EdicaoIDCP.AsInteger = 0) or (EdicaoSKU.AsString = EmptyStr) then
  begin
    DBGEdicaoSKU.Field.FocusControl;
    DataBaseError('Código do SKU não informado ou inválido !');
  end else

  if EdicaoDECP.AsString = EmptyStr then
  begin
    DBGEdicaoDECP.Field.FocusControl;
    DataBaseError('Descrição do produto não informado ou inválido !');
  end else

  if EdicaoQTDE.AsFloat = 0 then
  begin
    DBGEdicaoQTDE.Field.FocusControl;
    DataBaseError('Quantidade não informada ou inválida !');
  end else

  if EdicaoVPRC_COM.AsFloat = 0 then
  begin
    DBGEdicaoVPRC_COM.Field.FocusControl;
    DataBaseError('Preço comercial não informado ou inválido !');
  end;
end;
...
begin
  ...
  DBGEdicaoUCOM.DisableEditor := (EdicaoQTSP.AsFloat > 0);
...
begin
//  DBGPED_HST.ApplyBestFit(DBGPED_HSTDEPK);
  DBGPED_HST.ApplyBestFit(DBGPED_HSTSKU );
//  DBGPED_HST.ApplyBestFit(DBGPED_HSTDGCP);
end;

procedure Tfrmven_ped.PED_HSTAfterOpen(DataSet: TDataSet);
begin
  GBPED_HST.Caption := '  ' + EDDCLI.Text + '  ';
end;
