...
begin
  ...
  try
    ...
    begin
      try
        PESQUISA_CLIENTE('C',frmctr_ped.cadastroIDCD.AsString,0);
        ...
        edccli.Text := frmctr_ped.cadastroIDCD.AsString;
        ...
        cbcred.Tag  := frmctr_ped.cadastroTPCO.AsInteger;
        if frmctr_ped.cadastroTPCO.AsInteger = 0 then
        cbcred.Tag  := 1;
        edcdcx.Text := frmctr_ped.cadastroCDCX.AsString;
        eddcad.Date := strtodate(SLPrincipal.Values['data_sistema']);
        edhrom.Text := timetostr(frmctr_ped.cadastroHTPK.AsDateTime);
        edobse.Text := frmctr_ped.cadastroINFADCAD.AsString;

        edcven.Text := frmctr_ped.cadastroIDCV.AsString;
        edcrep.Text := frmctr_ped.cadastroIDCR.AsString;

        cbstpd.Text := frmctr_ped.cadastroSTPD.AsString;
        PESQUISA_TIPO;

        cbstfi.Text := frmctr_ped.cadastroDEST.AsString;
        ...
        edvfrt.Text := formatfloat('#,0.00',frmctr_ped.cadastroVFRT.AsFloat);

        latdsc.Caption := frmctr_ped.cadastroTDSC.AsString;
        edpdsc.Text    := formatfloat('#,0.00###',frmctr_ped.cadastroPDSC.AsFloat);
    ...
    try
      ...
      begin
        ...
        begin
          ...
          SQL.Add('AND    IDPK = ''' + Consulta_S.Fields[0].AsString + '''');
          ExecSQL;
        end;
        consulta_S.Next;
      end;
...
begin
  ...
  try
    ...
    begin
      ...
      SQL.Add('         PK.ROM_QTDE,PK.ROM_QTRL');
