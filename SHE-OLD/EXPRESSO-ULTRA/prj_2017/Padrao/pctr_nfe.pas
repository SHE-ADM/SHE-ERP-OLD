...
type
  Tfrmctr_nfe = class(Tfrmpadr1)
    ...
    SIVIS: TSpeedItem;
    SICLO: TSpeedItem;
    CadastroNFE_INDFINAL: TSmallintField;
    CadastroNFE_VTOTTRIB: TIBBCDField;
...
uses uPrincipal, pcad_cli, pcad_for, pven_npr, pemail, 
  pven_nfe, pnfe_cce, ppesquisa;
...
begin
  ...
  begin
    Close;
    SelectSQL.Clear;
    SelectSQL.Add('SELECT   EP.FANTASIA AS DEEP,PK.* FROM '+SLPrincipal.Values['nfe_cab'] + ' AS PK');
    SelectSQL.Add('JOIN     TAB_PAR_SIS AS EP ON (EP.ID = PK.IDEP)');
    SelectSQL.Add('WHERE    PK.NFE_DEMI BETWEEN '''+formatDateTime('mm/dd/yy',StartOfTheMonth(DATA_SISTEMA))+''' AND '''+formatDateTime('mm/dd/yy',endOfTheMonth(DATA_SISTEMA))+'''');
    SelectSQL.Add('ORDER BY PK.ID DESC');
    Prepare;
  end;
end;
...
begin
  ...
  begin
    Close;
    Params[0].Value := CadastroDEEP.AsString;
    Open;
  end;
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              SelectSQL.Add('AND PK.'+cField+' = '''+edtxt.Text+'''')
              else
              SelectSQL.Add('AND PK.'+cField+' LIKE ''%'+edtxt.Text+'%''');
            end;

            if (dxDT1.Date > 0) and (dxDT2.Date > 0) then
               SelectSQL.Add('AND PK.'+cData+' BETWEEN '''+formatDateTime('mm/dd/yy',dxDT1.Date)+''' AND '''+formatDateTime('mm/dd/yy',dxDT2.Date)+'''');

            SelectSQL.Add('ORDER BY PK.'+cfield+' DESC');
          end else
          SelectSQL.Add('ORDER BY PK.ID DESC');
          Open;
        end;
...
begin
  ...
  begin
    ...
    begin
      ...
      SPEdicao.ParamByName('DESC').Value := fin_rec_baiFIN_TIPO.AsString;
      SPEdicao.ParamByName('DOCT').Value := doct;
      SPEdicao.ParamByName('DCAD').Value := drom;
...
procedure Tfrmctr_nfe.FormResize(Sender: TObject);
begin
  RECDefault.MainArea := True;
  inherited;
end;
