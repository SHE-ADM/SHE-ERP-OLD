...
procedure TFrmSHE_DEF_ADM._SetDockControl (const AValue: TdxCustomDockControl; AXYPOS: Integer = 0; ADirection: TDirection = lNone; ARepeat: boolean = False; AUpdateZones: Boolean = False);
begin
...
procedure TFrmSHE_DEF_ADM.VCDSPrincipal1RodapeLEUpdateDockZones(
  Sender: TdxCustomDockControl; AZones: TList);
var
  i: Word;
begin
...
procedure TFrmSHE_DEF_ADM.TCDSPrincipal1RodapeLEUpdateDockZones(
  Sender: TdxCustomDockControl; AZones: TList);
var
  i: Word;
begin
...
procedure TFrmSHE_DEF_ADM.DPPrincipal1RodapeLEBUpdateDockZones(
  Sender: TdxCustomDockControl; AZones: TList);
var
  i: Word;
begin
...
procedure TFrmSHE_DEF_ADM.TCDSPrincipal1RodapeUpdateDockZones(
  Sender: TdxCustomDockControl; AZones: TList);
var
  I: Integer;
begin
...
procedure TFrmSHE_DEF_ADM.ConsultaBeforeOpen(DataSet: TDataSet);
begin
  { PAGE DOCK MANAGER }
  TCDSPrincipal1ConsultaLD.ActiveChildIndex := 0;
  TCDSPrincipal1RodapeLE.ActiveChildIndex   := 0;
...
procedure TFrmSHE_DEF_ADM.ConsultaBeforeScroll(DataSet: TDataSet);
begin
  if (Consulta.State <> dsBrowse) or (Consulta.RecNo = 0) then
  Exit;
end;

procedure TFrmSHE_DEF_ADM.ConsultaAfterScroll(DataSet: TDataSet);
begin
  if (Consulta.State <> dsBrowse) or (Consulta.RecNo = 0) then
  Exit;
end;

procedure TFrmSHE_DEF_ADM.DTSConsultaDataChange(Sender: TObject;
  Field: TField);
begin
  if (Consulta.State <> dsBrowse) or (Consulta.RecNo = 0) then
  Exit;
