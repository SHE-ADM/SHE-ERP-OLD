...
type
  Tfrmpag_can = class(Tfrmpadr3)
    ...
    procedure siPENClick(Sender: TObject);
  private
...
uses p_funcoes, ppesquisa, uPrincipal;

{$R *.dfm}

procedure Tfrmpag_can.AjustaForm;
begin
...
procedure Tfrmpag_can.ABRE_TABELA;
begin
  with cadastro do
  begin
    SelectSQL.Clear;
...
procedure Tfrmpag_can.siPSQClick(Sender: TObject);
var
  bret: boolean;
begin
...
procedure Tfrmpag_can.FormCreate(Sender: TObject);
begin
  inherited;
  abre_tabela;
  ajustaform;
...
procedure Tfrmpag_can.cadastroAfterCancel(DataSet: TDataSet);
begin
  if tag = 0 then
  begin
    finA.Enabled := true;
...
procedure Tfrmpag_can.cadastroAfterDelete(DataSet: TDataSet);
begin
  tSHEILD.CommitRetaining;
end;

procedure Tfrmpag_can.cadastroAfterEdit(DataSet: TDataSet);
begin
  if tag = 0 then
  begin
    finA.Enabled := false;
...
procedure Tfrmpag_can.cadastroAfterOpen(DataSet: TDataSet);
begin
  if tag = 0 then
  begin
    finA.Enabled := true;
...
procedure Tfrmpag_can.cadastroBeforeCancel(DataSet: TDataSet);
begin
  if tag = 0 then
  begin
    finA.Enabled := true;
...
procedure Tfrmpag_can.cadastroAfterPost(DataSet: TDataSet);
var
  wRec: TBookMark;
begin
  tSHEILD.CommitRetaining;
...
procedure Tfrmpag_can.finAClick(Sender: TObject);
begin
  cadastro.Edit;
end;

procedure Tfrmpag_can.finDClick(Sender: TObject);
begin
  cadastro.Delete;
end;

procedure Tfrmpag_can.siSAVClick(Sender: TObject);
begin
  cadastro.Post;
end;

procedure Tfrmpag_can.finCClick(Sender: TObject);
begin
  cadastro.Cancel;
end;

procedure Tfrmpag_can.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  case key of
...
procedure Tfrmpag_can.dbgConsultaKeyDown(Sender: TObject;
...
procedure Tfrmpag_can.siVALClick(Sender: TObject);
begin
  if cadastro.State in [dsEdit,dsInsert] then
     cadastro.Post;

  if yesno(handle,'Validar cancelamento dos títulos relacionados ?') = mrno then
     abort;
  ...
  try
    ...
    begin
      ...
      begin
        if (cadastroFIN_STFI.AsString = 'PENDENTE') or (cadastroFIN_STFI.AsString = 'CANCELADO') or (cadastroFIN_STFI.AsString = 'SUSPENSO') then
        begin
          ...
          begin
            ...
            SQL.Add('      FIN_DCAN = '''+formatDateTime('mm/dd/yy',strtodate(TSConsulta.Values['data_sistema']))+''',');
            SQL.Add('      FIN_VPAG = 0,');
            SQL.Add('      FIN_VPEN = '''+strtran(floattostr(cadastroFIN_VALO.AsFloat+cadastroFIN_VENC.AsFloat),',','.')+''',');
            ...
            SQL.Clear;
            SQL.Add('DELETE FROM '+TSConsulta.Values['fin_pag_rec']+' "FIN_PAG_REC"');
...
procedure Tfrmpag_can.cadastroFIN_VPAGValidate(Sender: TField);
begin
  cadastroFIN_VPEN.Value := (cadastroFIN_VALO.AsFloat+cadastroFIN_VENC.AsFloat)-cadastroFIN_VPAG.AsFloat;
end;

procedure Tfrmpag_can.dbgConsultaCustomDrawCell(Sender: TObject;
...
procedure Tfrmpag_can.cadastroBeforePost(DataSet: TDataSet);
begin
...
procedure Tfrmpag_can.fin_pagAfterOpen(DataSet: TDataSet);
begin
...
procedure Tfrmpag_can.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  inherited;
  if frmpag_can <> nil then
  begin
    frmpag_can.Release;
    frmpag_can := nil;
  end;
...
procedure Tfrmpag_can.FormCloseQuery(Sender: TObject;
...
procedure Tfrmpag_can.siMARClick(Sender: TObject);
begin
  if yesno(handle,'Deseja cancelar todos os lançamentos ?') = mrno then
  abort;
...
procedure Tfrmpag_can.siDESClick(Sender: TObject);
begin
  if yesno(handle,'Deseja suspender todos os lançamentos ?') = mrno then
  abort;
  ...
  try
    ...
    begin
      ...
      cadastroFIN_VPEN.Value    := cadastroFIN_VALO.AsFloat;
      cadastroFIN_DPAG.AsString := '';
      cadastroFIN_STFI.Value    := 'SUSPENSO';
      cadastro.Post;
      cadastro.Next;
    end;
  finally
...
procedure Tfrmpag_can.siPENClick(Sender: TObject);
begin
  if yesno(handle,'Deseja marcar todos os lançamentos como pendente ?') = mrno then
  abort;
