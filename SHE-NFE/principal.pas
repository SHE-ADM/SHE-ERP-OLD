...
uses
  ...
  DBGrids, ExtCtrls, Buttons, Registry, jpeg, StrInt, StrIntImp, pngimage;
...
type
  ...
  TFrmPrincipal = class(TForm)
    ...
    Label74: TLabel;
    Label75: TLabel;
    mmoLogInsucessoEntrega: TMemo;
    edtInsEntrNfe: TEdit;
    edtInsEntrDh: TEdit;
    edtInsEntrQtdTent: TEdit;
    edtInsEntrCodMot: TEdit;
    btnInsucessoEntrega: TButton;
    Label76: TLabel;
    edtInsEntrMotivo: TEdit;
    Label77: TLabel;
...
try
 ...
 SetLength(tProd, qtdItens, 297); //prod
 ...
 tEmit[0]:='Emitente ASX Comp test';                    //<xNome>
 tEmit[1]:='ASX Test';                                  //<xFant>
...
{ Tag da Declaração de Importação (Ocorrência 0-n). Se houver declaração de inportação informar. Para muitas ocorrências, informe os dados separados por ";"}
  ...
  { Grupo do detalhamento de Medicamentos e de matérias primas farmacêuticas.
   ...
   tProd[x, 132]:= '';              //Excluído no leiaute 4.0 - NT2016.002) <pMixGN> Percentual de Gás Natural para o produto GLP (cProdANP=210203001)
  ...
  //NOTA: Caso este item nao possua dados de veiculo, informe: tProd[x,82]:='';
    ...
    tProd[x,258] := '';                                  //gIBSUF <vIBSUF> Valor do IBS de competência da UF

    {Grupo de Informações do IBS para o município <gIBSCBS/gIBSMun>}

    tProd[x,259] := '';                                  //gIBSMun <pIBSMun> Alíquota do IBS de competência do Município
    ...
    tProd[x,265] := '';                                  //gIBSMun <vIBSMun> Valor do IBS de competência do Município

    tProd[x,266] := '';                                  //gIBSCBS <vIBS> Valor do IBS

    {Grupo de informações da CBS}

    tProd[x,267] := '';                                  //gCBS <pCBS> Alíquota da CBS
    ...
    tProd[x,273] := '';                                  //gCBS <vCBS> Valor da CBS
    ...
    tProd[x,238] := '';         //<vItem> Valor total do Item, correspondente à sua participação no total da nota. A soma dos itens deverá corresponder ao total da nota.
 ...
 tTotal[42]:='';     //qBCMono
 tTotal[43]:='';     //vICMSMono
 tTotal[44]:='116.00';     //qBCMonoReten
 tTotal[45]:='145.82';     //vICMSMonoReten
 ...
 tTotal[49] := '';                 //<vBCIBSCBS> Valor total da BC do IBS e da CBS
 ...
 tTotal[50] := '';                 //<vDif> Valor total do diferimento
 tTotal[51] := '';                 //<vDevTrib> Valor total de devolução de tributos
 tTotal[52] := '';                 //<vIBSUF> Valor total do IBS da UF

 {<gIBSMun> Grupo Total do IBS do Município}

 tTotal[53] := '';                 //<vDif> Valor total do diferimento
 tTotal[54] := '';                 //<vDevTrib> Valor total de devolução de tributos
 tTotal[55] := '';                 //<vIBSMun> Valor total do IBS da UF

 tTotal[56] := '';                 //<vIBS> Valor total do IBS
 tTotal[57] := '';                 //<vCredPres> Valor total do crédito presumido
 tTotal[58] := '';                 //<vCredPresCondSus> Valor total do crédito presumido em condição suspensiva

 {<gCBS> Grupo Total da CBS}
 {Para gerar este grupo todas tags abaixo devem ser informadas}

 tTotal[59] := '';                 //<vDif> Valor total do diferimento
 tTotal[60] := '';                 //<vDevTrib> Valor total de devolução de tributos
 tTotal[61] := '';                 //<vCBS> Valor total da CBS
 tTotal[62] := '';                 //<vCredPres> Valor total do crédito presumido
 tTotal[63] := '';                 //<vCredPresCondSus> Valor total do crédito presumido em condição suspensiva
 ...
 tCobr[0]:='001';                //fat <nFat> Número da Fatura
 tCobr[1]:='20.00';                //fat <vOrig> Valor Original da Fatura
 tCobr[2]:='20.00';                //fat <vLiq> Valor Líquido da Fatura
 tCobr[10]:='0.00';                //fat <vDesc> Valor do desconto

 { 3 parcelas neste caso ** a partir da parcela 24ª, preencher nas posicoes tCobr 5, 7 e 8}
 tCobr[3]:='001';           //dup <nDup> Número da Duplicata
 tCobr[4]:='2025-08-29';           //dup <dVenc> Data de vencimento - Formato: “AAAA-MM-DD”
 tCobr[5]:='20.00';           //dup <vDup> Valor da duplicata
 ...
 tPag[1]:= ';';             //<vPag> Valor do Pagamento
 ...
 tPag[7]:= '0';                   //<indPag> Indicador de pgto 0|1
 tPag[8]:= '';                    //<xPag> 'descrição do meio de pagamento' (xPag) - NT 2020.006 v1.20
 tPag[9]:= '';                    //<dPag> Data do Pagamento
...
begin
 ...
 Caption := 'ASX COMPONENTES | API NFEDLL ' + VersaoDLL;
...
procedure TFrmPrincipal.btnInsucessoEntregaClick(Sender: TObject);
var
 str : string;
begin

 btnInsucessoEntrega.enabled:=false;
 Screen.Cursor:=crAppStart;
 EditStatus.Text:='Aguarde...';


 if (edtInsEntrNfe.Text <> '') then
  begin
     mmoLogInsucessoEntrega.Lines.Add('## Metodo: Insucesso de Entrega');
     mmoLogInsucessoEntrega.Lines.Add('Retorno:');

     str := mmoLogInsucessoEntrega.text;


     mmoLogInsucessoEntrega.Lines.Add(
      EventoEntregaInsucesso( '1',    //Identificador de controle do Lote de envio do Evento
      edtInsEntrNfe.Text,  //Chave de Acesso da NF-e à qual o evento será vinculado
      '1',                   //Sequencial do evento para o mesmo tipo de evento.
      edtInsEntrDh.Text,     ///Data e hora da tentativa de entrega. Formato AAAA-MMDDThh:mm:ssTZD
      edtInsEntrQtdTent.Text,    //Número da tentativa de entrega que não teve sucesso
      edtInsEntrCodMot.Text,   //Código Motivo do insucesso
      edtInsEntrMotivo.Text,   //Justificativa do motivo do insucesso. Informar apenas para tpMotivo=4
      ''));                   //Latitude e Longitude (ex: -23.573593, -46.706675)

      // Código Motivo do insucesso:
 ...
 btnInsucessoEntrega.Enabled:=true;
 EditStatus.Text:='Concluído.';
 Screen.Cursor:=crDefault;
end;
