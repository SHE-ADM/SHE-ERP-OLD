...
type
  TFrmPED_PDC_ADM = class(TForm)
    ...
    DBGConsultaDSNF: TdxDBGridColumn;
    DBGConsultaDSSA: TdxDBGridDateColumn;
    ...
    ConsultaVCOM: TIBBCDField;
    ConsultaPCOM: TIBBCDField;
    ConsultaMFRT: TSmallintField;
    ConsultaDFRT: TIBStringField;
    ConsultaVFRT: TIBBCDField;
    ConsultaPSBR: TIBBCDField;
    ConsultaPSLQ: TIBBCDField;
    ConsultaINFADCPL: TIBStringField;
    ConsultaDSOS: TDateTimeField;
    ConsultaDSRO: TDateTimeField;
    ConsultaDSNF: TDateTimeField;
    ConsultaDSSA: TDateTimeField;
    ...
    DBGConsultaDTBX: TdxDBGridColumn;
    DBGConsultaRECO: TdxDBGridMaskColumn;
    ConsultaSP_VTAP: TIBBCDField;
    ConsultaSP_RLAP: TIntegerField;
    DBGConsultaSP_VTAP: TdxDBGridMaskColumn;
    DBGConsultaSP_RLAP: TdxDBGridColumn;
    DBGConsultaSP_TCAP: TdxDBGridMaskColumn;
    DBGConsultaSP_NCAP: TdxDBGridCurrencyColumn;
    DBGConsultaRLAP: TdxDBGridColumn;
    ConsultaSP_TCAP: TIBStringField;
    ConsultaSP_NCAP: TIBBCDField;
    ConsultaRLLQ: TIntegerField;
    ConsultaRLDV: TIntegerField;
    ConsultaLQ_RLDV: TIntegerField;
    DBGConsultaRLLQ: TdxDBGridColumn;
    DBGConsultaLQ_RLAP: TdxDBGridColumn;
    DBGConsultaRLDV: TdxDBGridColumn;
    DBGConsultaLQ_VTDV: TdxDBGridColumn;
    DBGConsultaLQ_RLDV: TdxDBGridColumn;
    DBGConsultaLQ_VTAB: TdxDBGridColumn;
...
var
  FrmPED_PDC_ADM: TFrmPED_PDC_ADM;
...
procedure TFrmPED_PDC_ADM._WM_CREATE(var Msg: TMessage);
begin
...
procedure TFrmPED_PDC_ADM._SW_SHOWNOACTIVATE(var Msg: TMessage);
...
procedure TFrmPED_PDC_ADM._WM_ACTIVATE(var Msg: TMessage);
begin
...
procedure TFrmPED_PDC_ADM._WM_BEFORE_RESIZE(var Message: TMessage);
begin
  { Before Resize }
...
procedure TFrmPED_PDC_ADM._WM_AFTER_RESIZE(var Message: TMessage);
begin
  { After Resize }
...
procedure TFrmPED_PDC_ADM._SetCurrentEvent(const AValue: String);
begin
  FCurrentEvent := AValue;
end;

procedure TFrmPED_PDC_ADM._SetForceClose(const AValue: Boolean);
begin
  FForceClose := AValue;
end;

Class procedure TFrmPED_PDC_ADM._ExecForm(AOwner: TComponent;var AForm; AFormPesquisa: Boolean = False; AFormStyle: TFormStyle = fsMDIChild;
                                          AIDEP : Integer = 0;
                                          AIDPK : LongInt = 0;
                                          AIDEV : LongInt = 0;
                                          ACDEV : Word    = 0;
...
Destructor TFrmPED_PDC_ADM.Destroy;
type
  PtrForm = ^TForm;
var
  idxForm: Integer;
...
Constructor TFrmPED_PDC_ADM.Create(AOwner: TComponent;
                              const AIDEP: Integer;
                                    AIDPK: LongInt;
                                    AIDEV: LongInt;
                                    ACDEV: Word;
...
procedure TFrmPED_PDC_ADM.FormCreate(Sender: TObject);
begin
  { ADMIN }
  DBGConsultaIDPK.Visible := (RECUsuarios.ID = 0);
  DBGConsultaDTBX.Visible := (RECUsuarios.ID = 0);
...
procedure TFrmPED_PDC_ADM.FormShow(Sender: TObject);
begin
  OnShow := Nil;
...
procedure TFrmPED_PDC_ADM.FormActivate(Sender: TObject);
begin
  OnActivate := Nil;
  if REC_SHE_DEF.FForceClose then
  Exit;
...
procedure TFrmPED_PDC_ADM.FormClose(Sender: TObject;
...
procedure TFrmPED_PDC_ADM.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  case key of
...
procedure TFrmPED_PDC_ADM.FormKeyPress(Sender: TObject; var Key: Char);
begin
  if key = #5 then
  {[Ctrl+E]} else
  if key = #8 then
...
procedure TFrmPED_PDC_ADM.FormPaint(Sender: TObject);
var
...
procedure TFrmPED_PDC_ADM.FormResize(Sender: TObject);
begin
  if (not Showing) or (REC_SHE_DEF.FForceClose) then
  Exit;
...
procedure TFrmPED_PDC_ADM._SetDockControl (const AValue: TdxCustomDockControl; AXYPos: Integer = 0; ADirection: TDirection = lNone; ARepeat: boolean = False; AUpdateZones: Boolean = False);
var
...
procedure TFrmPED_PDC_ADM.VCDSPrincipal1RodapeLEUpdateDockZones(
  Sender: TdxCustomDockControl; AZones: TList);
var
  i: Word;
begin
...
procedure TFrmPED_PDC_ADM.TCDSPrincipal1RodapeLEUpdateDockZones(
  Sender: TdxCustomDockControl; AZones: TList);
var
  i: Word;
begin
...
procedure TFrmPED_PDC_ADM.DPPrincipal1RodapeLEBUpdateDockZones(
  Sender: TdxCustomDockControl; AZones: TList);
var
  i: Word;
begin
...
procedure TFrmPED_PDC_ADM.TCDSPrincipal1RodapeUpdateDockZones(
  Sender: TdxCustomDockControl; AZones: TList);
var
  I: Integer;
begin
...
procedure TFrmPED_PDC_ADM.ACTRefreshExecute(Sender: TObject);
begin
  oRefresh(Consulta);
end;

procedure TFrmPED_PDC_ADM.ACTMAppendExecute(Sender: TObject);
begin
...
procedure TFrmPED_PDC_ADM.ACTMEditExecute(Sender: TObject);
begin
...
procedure TFrmPED_PDC_ADM.ACTMDeleteExecute(Sender: TObject);
begin
...
procedure TFrmPED_PDC_ADM.ACTMPostExecute(Sender: TObject);
begin
...
procedure TFrmPED_PDC_ADM.ACTMCancelExecute(Sender: TObject);
begin
...
procedure TFrmPED_PDC_ADM.ACTSaidaExecute(Sender: TObject);
begin
  REC_SHE_DEF.PSQ_OK := False;
  Close;
end;

procedure TFrmPED_PDC_ADM.ACTEAppendExecute(Sender: TObject);
begin
...
procedure TFrmPED_PDC_ADM.ACTEEditExecute(Sender: TObject);
begin
...
procedure TFrmPED_PDC_ADM.ACTEDeleteExecute(Sender: TObject);
begin
...
procedure TFrmPED_PDC_ADM.ACTEPostExecute(Sender: TObject);
begin
...
procedure TFrmPED_PDC_ADM.ACTECancelExecute(Sender: TObject);
begin
...
procedure TFrmPED_PDC_ADM.ACTPesquisaExecute(Sender: TObject);
var
  i: Integer;
begin
  { RESET TRANSACTION }
  ...
  try
    ...
    begin
      ...
      SQL.Add('SELECT PK.ID  ,PK.EF_IDEP,');
      SQL.Add('       PK.DTCA,PK.DTEV,');
      SQL.Add('       PK.CDST,PK.REST,PK.DEST,PK.STFI,PK.APST,PK.FPST,PK.FAST,PK.LQST,PK.BQST,');

      SQL.Add('       PK.IDPK,PK.DEPK,PK.DTPK,PK.HTPK,PK.CDCX,');
      SQL.Add('       PK.CDOS,PK.DSOS,');
      SQL.Add('       PK.CDSP,PK.LGSP,PK.DTSP  ,PK.HTSP,');
      SQL.Add('       NULLIF(PK.CDRO,0) AS CDRO,PK.DSRO,');
      SQL.Add('       NULLIF(PK.CDNF,0) AS CDNF,PK.DSNF,');
      SQL.Add('       PK.IDSA,PK.DSSA,');
      SQL.Add('       PK.CDBX,PK.DTBX,');
      SQL.Add('       PK.VCOM,PK.PCOM,');
      ...
      SQL.Add('       PK.MFRT,PK.DFRT,PK.VFRT,PK.PSBR,PK.PSLQ,');

      SQL.Add('       PK.VDSC,PK.PDSC,');
      SQL.Add('       PK.TCDE,PK.QTRL,PK.LQ_TCDE,PK.LQ_QTRL,');
      SQL.Add('       PK.VTSP,PK.RLSP,PK.PPSP   ,PK.LQ_VTSP,PK.LQ_RLSP,PK.LQ_PPSP,');
      SQL.Add('       PK.VTLQ,PK.RLLQ,PK.VTBQ,');
      SQL.Add('       PK.VTAP,PK.RLAP,PK.LQ_VTAP,PK.LQ_RLAP,');
      SQL.Add('       PK.SP_VTAP,PK.SP_RLAP,PK.SP_TCAP,PK.SP_NCAP,');
      SQL.Add('       PK.VTDV,PK.RLDV,PK.LQ_VTDV,PK.LQ_RLDV,');
      SQL.Add('       PK.VTAB,PK.LQ_VTAB,');
...
procedure TFrmPED_PDC_ADM.ACTPSQ_OKExecute(Sender: TObject);
begin
...
procedure TFrmPED_PDC_ADM.ACTPSQ_FOCUSExecute(Sender: TObject);
begin
  if (ACTPesquisa.Enabled) and (BEPSQ_CAD.Enabled) then
  BEPSQ_CAD.SetFocus(False);
end;

procedure TFrmPED_PDC_ADM.BEPSQ_CADKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrmPED_PDC_ADM.BEPSQ_CADCurChange(Sender: TObject);
begin
  BEPSQ_CAD.Text := BEPSQ_CAD.CurText;
end;

procedure TFrmPED_PDC_ADM.BDPSQ_PER_INICurChange(Sender: TObject);
begin
  BDPSQ_PER_INI.Date := BDPSQ_PER_INI.CurDate;
end;

procedure TFrmPED_PDC_ADM.BDPSQ_PER_FIMCurChange(Sender: TObject);
begin
  BDPSQ_PER_FIM.Date := BDPSQ_PER_FIM.CurDate;
end;

procedure TFrmPED_PDC_ADM.ACTExecEventExecute(Sender: TObject);
var
  i: word;
begin
  if REC_SHE_DEF.FB_Event <> EmptyStr then
...
procedure TFrmPED_PDC_ADM.EEventEventAlert(Sender: TObject;
  EventName: String; EventCount: Integer; var CancelAlerts: Boolean);
begin
  oRefresh(Consulta);
end;

procedure TFrmPED_PDC_ADM.ConsultaBeforeOpen(DataSet: TDataSet);
begin
  DBGConsultaSTPD.Visible := False; { Tipos Pedidos }
  DBGConsultaDSNF.Visible := False; { Nota Fiscal }
  ...
  { Em Aberto }
  DBGConsultaSP_VTAP.Visible := False;
  DBGConsultaSP_RLAP.Visible := False;
  DBGConsultaSP_TCAP.Visible := False;
...
procedure TFrmPED_PDC_ADM.ConsultaBeforeClose(DataSet: TDataSet);
begin
  PED_PDV_ITE.Close; { Pedido }
  PED_PDV_SEP.Close; { Expedição }
end;

procedure TFrmPED_PDC_ADM.ConsultaAfterOpen(DataSet: TDataSet);
begin
...
procedure TFrmPED_PDC_ADM.ConsultaBeforeScroll(DataSet: TDataSet);
begin
  { PED_PDV_ITE  }
  DBGPED_PDV_ITE.Bands[03].Visible := False; { Expedição }
  DBGPED_PDV_ITE.Bands[04].Visible := False; { OS }
  DBGPED_PDV_ITE.Bands[05].Visible := False; { Descontos }
...
procedure TFrmPED_PDC_ADM.ConsultaAfterScroll(DataSet: TDataSet);
begin
  { Vendedores }
  ...
  { Em Aberto }
  if ConsultaSP_VTAP.AsFloat > 0 then
  begin
    DBGConsultaSP_VTAP.Visible := True;
    DBGConsultaSP_RLAP.Visible := True;
    DBGConsultaSP_TCAP.Visible := True;
  end;
...
procedure TFrmPED_PDC_ADM.ConsultaCalcFields(DataSet: TDataSet);
begin
  ConsultaC_ID.Value := Consulta.RecNo;
end;

procedure TFrmPED_PDC_ADM.DTSConsultaDataChange(Sender: TObject;
  Field: TField);
var
  APosCount,
  APosHeight: Word;
begin
  ...
  begin
    ...
    APosCount := oPosCount(''#$D'',ConsultaINFADCPL.AsString);

    { VER TAM TELA }
    { Altura }
    APosHeight :=

    IFThen((APosCount = 00) and (ConsultaINFADCPL.AsString =  EmptyStr),30,
    IFThen((APosCount = 00) and (ConsultaINFADCPL.AsString <> EmptyStr),30,
    ...
    if DBGConsultaGPCD.Width  > 90 then
       DBGConsultaGPCD.Width := 90;
    ...
    if DBGConsultaDECO.Width > 150 then
       DBGConsultaDECO.Width := IFThen(Screen.Width <= 1366,100,
    ...
    DBGConsultaDSNF.Visible := True;

    { Saídas }
    if ConsultaDSSA.AsDateTime > 0 then
...
procedure TFrmPED_PDC_ADM.DBGConsultaCustomDrawCell(Sender: TObject;
  ACanvas: TCanvas; ARect: TRect; ANode: TdxTreeListNode;
...
begin
  ...
  begin
    ...
    { Faturados }
    if (AColumn = DBGConsultaVTLQ) or (AColumn = DBGConsultaRLLQ) then
    ...
    { Abertos Padrão }
    if (AColumn = DBGConsultaVTAP) or (AColumn = DBGConsultaRLAP) then
    ...
    { Abertos Separados}
    if (AColumn = DBGConsultaSP_VTAP) or (AColumn = DBGConsultaSP_RLAP) or (AColumn = DBGConsultaSP_TCAP) then
    if (ANode.Values[DBGConsultaSP_VTAP.Index] > 0) then
    begin
      AColor      := $00F5FFFF;
      AFont.Color := clWindowText;
    end else
    ...
    { Devoluções }
    if (AColumn = DBGConsultaVTDV) or (AColumn = DBGConsultaRLDV) then
    ...
    { Romaneio }
    if AColumn = DBGConsultaCDRO then
    begin
      //AColor      := $00F5F5F5;
      AFont.Color := clWindowText;
    end;
...
procedure TFrmPED_PDC_ADM.DBGConsultaKeyDown(Sender: TObject;
...
procedure TFrmPED_PDC_ADM.PED_PDV_ITEAfterOpen(DataSet: TDataSet);
begin
  if Consulta.State    <> dsInactive then
  if PED_PDV_ITE.State <> dsInactive then
...
procedure TFrmPED_PDC_ADM.PED_PDV_ITEBeforeClose(DataSet: TDataSet);
begin
  CAD_PRO_IMG.Close; { Catálogos }
end;

procedure TFrmPED_PDC_ADM.PED_PDV_ITEAfterScroll(DataSet: TDataSet);
begin
  { DOCK MANAGER }
  ...
  begin
    ...
    begin
      DBGPED_PDV_ITE.Bands[03].Visible := True; { OS }
      DBGPED_PDV_ITE.Bands[04].Visible := True; { Expedição }
    end;

    { Descontos }
    if PED_PDV_ITEVDSC.AsFloat > 0 then
    DBGPED_PDV_ITE.Bands[05].Visible := True;
...
procedure TFrmPED_PDC_ADM.PED_PDV_ITECalcFields(DataSet: TDataSet);
begin
...
procedure TFrmPED_PDC_ADM.DTSPED_PDV_ITEDataChange(Sender: TObject;
  Field: TField);
begin
  if Consulta.State    <> dsInactive then
  if PED_PDV_ITE.State <> dsInactive then
...
procedure TFrmPED_PDC_ADM.DTSCAD_PRO_IMGDataChange(Sender: TObject;
  Field: TField);
begin
  if Consulta.State    <> dsInactive then
  if PED_PDV_ITE.State <> dsInactive then
...
procedure TFrmPED_PDC_ADM.DBGPED_PDV_ITECustomDrawCell(Sender: TObject;
  ACanvas: TCanvas; ARect: TRect; ANode: TdxTreeListNode;
  AColumn: TdxTreeListColumn; ASelected, AFocused, ANewItemRow: Boolean;
  var AText: String; var AColor: TColor; AFont: TFont;
  var AAlignment: TAlignment; var ADone: Boolean);
BEGIN
  ...
  begin
    ...
    { Itens }
    if AColumn = DBGPED_PDV_ITEITEM then
    begin
      AColor      := $00F5F5F5;
      AFont.Color := clWindowText;
    end;
    ...
    { Produtos }
    if AColumn = DBGPED_PDV_ITETCDE then
    begin
      AColor      := $00FAFAFA;
      AFont.Color := clWindowText;
    end;
...
procedure TFrmPED_PDC_ADM.PED_PDV_SEPCalcFields(DataSet: TDataSet);
begin
