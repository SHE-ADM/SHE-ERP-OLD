...
type
  TFBird = class(TDataModule)
    ...
    QFBResumo_MensalEMPRESA: TIBStringField;
    QFBResumo_MensalPERIODO: TIBStringField;
    QFBResumo_MensalPVCLI: TLargeintField;
    QFBResumo_MensalNVCLI: TLargeintField;
    QFBResumo_MensalQTVEN: TLargeintField;
    QFBResumo_MensalVTVEN: TIBBCDField;
    QFBResumo_MensalVTDEV: TIBBCDField;
    DTSFBResumo_Mensal: TDataSource;
    DTSFBResumo_Diario: TDataSource;
    QFBResumo_Diario: TIBQuery;
    QFBResumo_DiarioEMPRESA: TIBStringField;
    QFBResumo_DiarioPERIODO: TDateField;
    QFBResumo_DiarioPVCLI: TLargeintField;
    QFBResumo_DiarioNVCLI: TLargeintField;
    QFBResumo_DiarioQTVEN: TLargeintField;
    QFBResumo_DiarioVTVEN: TIBBCDField;
    QFBResumo_DiarioVTDEV: TIBBCDField;
    ...
    QFBResumo_MensalPERIODO_EXT: TIBStringField;
    ...
    QFBResumo_MensalVTFAT: TFloatField;
    QFBResumo_DiarioVTFAT: TFloatField;
    ...
    CDSFBUsuariosID: TIntegerField;
    CDSFBUsuariosUSU_CUSU: TIntegerField;
    CDSFBUsuariosUSU_FUNC: TStringField;
    CDSFBUsuariosUSU_ROTI: TStringField;
    CDSFBUsuariosUSU_NOME: TStringField;
    CDSFBUsuariosUSU_NOVO: TStringField;
    CDSFBUsuariosUSU_EDIT: TStringField;
    CDSFBUsuariosUSU_DELE: TStringField;
    CDSFBUsuariosUSU_VISU: TStringField;
    CDSFBUsuariosUSU_PRIN: TStringField;
    CDSFBUsuariosUSU_AUTO: TStringField;
    CDSFBUsuariosUSU_CTRL: TStringField;
    CDSFBUsuariosUSU_PDSC: TStringField;
    CDSFBUsuariosUSU_PCRE: TStringField;
    CDSFBUsuariosUSU_MCRE: TStringField;
    CDSFBUsuariosUSU_FINA: TSmallintField;
    CDSFBUsuariosFLAG: TSmallintField;
...
function  bPSQLOG (ACampo,ADescricao,AReferencia,ARegra: String; AMensagem: Boolean = False): Boolean; STDCall;
...
begin
  ...
  try
    ...
    begin
      Close;
      SQL.Clear;
      SQL.Add('SELECT * FROM VW_TAB_USER');
...
begin
  ...
  with FBird do
  begin
    //Result := (Pos(RECUsuarios.Grupo,'DEVTI') > 0);

    if Not Result then
    begin
      CDSFBUsuarios.First;
      if CDSFBUsuarios.Locate('USU_FUNC;USU_ROTI;USU_NOME', VarArrayOf([AFuncao,ARotina,ANome]),[]) then
         Result := (CDSFBUsuarios.FieldByName(ACampo).AsString = '1');
    end;
  end;

  if (not Result) and (AMensagem) then
...
begin
  ...
  if (RECUsuarios.ID > 0) and (RECParametros.ID > 0) then
     with FBird do
     begin
       CDSTAB_USER_ADM.First;
       CDSTAB_USER_ADM.Locate('Descricao;Referencia;Regra',VarArrayOf([ADescricao,AReferencia,ARegra]),[]);

       Result := (CDSTAB_USER_ADM.FieldByName(ACampo).AsInteger = 1);
    end;
...
begin
  ...
  begin
    ...
    begin
      Close;
      SQL.Clear;
      SQL.Add('SELECT EMPRESA,PERIODO,');
      SQL.Add('       TRIM(CASE LEFT(PERIODO,3)');
      SQL.Add('              WHEN ''01/'' THEN ''JANEIRO  ''');
      SQL.Add('              WHEN ''02/'' THEN ''FEVEREIRO''');
      SQL.Add('              WHEN ''03/'' THEN ''MARÇO    ''');
      SQL.Add('              WHEN ''04/'' THEN ''ABRIL    ''');
      SQL.Add('              WHEN ''05/'' THEN ''MAIO     ''');
      SQL.Add('              WHEN ''06/'' THEN ''JUNHO    ''');
      SQL.Add('              WHEN ''07/'' THEN ''JULHO    ''');
      SQL.Add('              WHEN ''08/'' THEN ''AGOSTO   ''');
      SQL.Add('              WHEN ''09/'' THEN ''SETEMBRO ''');
      SQL.Add('              WHEN ''10/'' THEN ''OUTUBRO  ''');
      SQL.Add('              WHEN ''11/'' THEN ''NOVEMBRO ''');
      SQL.Add('              WHEN ''12/'' THEN ''DEZEMBRO ''');
      SQL.Add('            END ) AS PERIODO_EXT,');
      SQL.Add('       SUM(PVCLI) AS PVCLI,SUM(NVCLI) AS NVCLI,');
      SQL.Add('       SUM(QTVEN) AS QTVEN,SUM(VTVEN) AS VTVEN,SUM(VTDEV) AS VTDEV,SUM(VTFAT-VTDEV) AS VTFAT');
      SQL.Add('FROM (');

      { Pedidos }
      SQL.Add('SELECT   PS.FANTASIA AS EMPRESA,');
      SQL.Add('         LPad(Extract(Month FROM CAST(PK.DTCA AS DATE)),2,0)||''/''||RIGHT(LPad(Extract(Year  FROM CAST(PK.DTCA AS DATE)),4,0),2) PERIODO,');
      SQL.Add('         COUNT(DISTINCT (IIF(LPad(Extract(Month FROM CAST(CD.DTCA AS DATE)),2,0)||''/''||LPad(Extract(Year  FROM CAST(CD.DTCA AS DATE)),4,0) = ');
      SQL.Add('         LPad(Extract(Month FROM CAST(PK.DTCA AS DATE)),2,0)||''/''||LPad(Extract(Year  FROM CAST(PK.DTCA AS DATE)),4,0),PK.IDCD,NULL))) AS NVCLI,');
      SQL.Add('         COUNT(DISTINCT PK.IDCD) PVCLI,');
      SQL.Add('         COUNT(DISTINCT FK.IDPK) QTVEN,');
      SQL.Add('         COALESCE(SUM(IIF(PK.STPD <> ''DEVOLUÇÃO'',FK.TCDE,0)),0) AS VTVEN,');
      SQL.Add('         COALESCE(SUM(IIF(PK.STPD =  ''DEVOLUÇÃO'',FK.TCDE*IIF(PK.TPCO > 0,PK.TPCO,1),0)),0) AS VTDEV,');
      SQL.Add('         0 AS VTFAT');
      SQL.Add('FROM   '+SLPrincipal.Values['ped_ven_cab']+' AS PK');
      SQL.Add('JOIN   '+SLPrincipal.Values['ped_ven_ite']+' AS FK ON (FK.IDPK = PK.IDPK)');
      SQL.Add('JOIN     CAD_CLI     AS CD ON (CD.ID = PK.IDCD   )');
      SQL.Add('JOIN     TAB_PAR_SIS AS PS ON (PS.ID = FK.CP_IDEP)');
      SQL.Add('WHERE    PK.IDEP = '''+RECParametros.Id+'''');
      SQL.Add('AND      PK.FAPD = 1');
      SQL.Add('AND      PK.STPD <> ''ABATIMENTO''');
      SQL.Add('AND      CAST(PK.DTCA AS DATE) >= CAST(''01/01/''||EXTRACT(YEAR FROM CURRENT_DATE) AS DATE)');
      SQL.Add('GROUP BY EMPRESA,PERIODO');

      { Romaneios }
      SQL.Add('UNION');
      SQL.Add('SELECT   PS.FANTASIA AS EMPRESA,');
      SQL.Add('         LPad(Extract(Month FROM CAST(PK.DTNF AS DATE)),2,0)||''/''||RIGHT(LPad(Extract(Year FROM CAST(PK.DTNF AS DATE)),4,0),2) PERIODO,');
      SQL.Add('         0 AS NVCLI,0 PVCLI,0 QTVEN,0 AS VTVEN,0 AS VTDEV,');
      SQL.Add('         COALESCE(SUM(FK.VPRC_COM*FK.QTDE*IIF(PK.TPCO > 0,PK.TPCO,1)),0) AS VTFAT');
      ...
      SQL.Add('WHERE    PK.IDEP = '''+RECParametros.Id+'''');
      SQL.Add('AND      PK.FAPD = 1');
      SQL.Add('AND      POSITION(LEFT(PK.STPD,3) IN ''DEVABA'') = 0');
      SQL.Add('AND      CAST(PK.DTNF AS DATE) >= CAST(''01/01/''||EXTRACT(YEAR FROM CURRENT_DATE) AS DATE)');
      SQL.Add('GROUP BY EMPRESA,PERIODO)');
      SQL.Add('GROUP BY EMPRESA,PERIODO ');
      SQL.Add('ORDER BY PERIODO DESC');
      Prepare;
      Open;
    end;
    ...
    begin
      ...
      { Pedidos }
      SQL.Add('SELECT   PS.FANTASIA AS EMPRESA,CAST(PK.DTCA AS DATE) AS PERIODO,');
      SQL.Add('         COUNT(DISTINCT (IIF(CAST(CD.DTCA AS DATE) = CAST(PK.DTCA AS DATE),CD.ID,NULL))) AS NVCLI,');
      SQL.Add('         COUNT(DISTINCT PK.IDCD) PVCLI,');
      SQL.Add('         COUNT(DISTINCT FK.IDPK    ) QTVEN,');
      SQL.Add('         COALESCE(SUM(IIF(PK.STPD <> ''DEVOLUÇÃO'',FK.TCDE,0)),0) AS VTVEN,');
      SQL.Add('         COALESCE(SUM(IIF(PK.STPD =  ''DEVOLUÇÃO'',FK.TCDE*IIF(PK.TPCO > 0,PK.TPCO,1),0)),0) AS VTDEV,');
      ...
      SQL.Add('AND      PK.FAPD = 1');
      SQL.Add('AND      PK.STPD <> ''ABATIMENTO''');
      SQL.Add('AND     (CAST(PK.DTCA AS DATE) BETWEEN (DATEADD(1 - EXTRACT(DAY FROM CURRENT_DATE) DAY TO CURRENT_DATE )) AND (DATEADD(-EXTRACT(DAY FROM DATEADD(1 MONTH TO CURRENT_DATE )) DAY TO DATEADD(1 MONTH TO CURRENT_DATE))))');
      SQL.Add('GROUP BY EMPRESA,PERIODO');

      { Romaneios }
      SQL.Add('UNION');
      SQL.Add('SELECT   PS.FANTASIA AS EMPRESA,CAST(PK.DTNF AS DATE) AS PERIODO,');
      SQL.Add('         0 AS NVCLI,0 PVCLI,0 QTVEN,0 AS VTVEN,0 AS VTDEV,');
      SQL.Add('         COALESCE(SUM(FK.VPRC_PAD*FK.QTDE*IIF(PK.TPCO > 0,PK.TPCO,1)),0) AS VTFAT');
      SQL.Add('FROM   '+SLPrincipal.Values['rom_cab']+' AS PK');
      SQL.Add('JOIN   '+SLPrincipal.Values['rom_ite']+' AS FK ON (FK.CDRO = PK.CDRO)');
      SQL.Add('JOIN     CAD_CLI     AS CD ON (CD.ID = PK.IDCD   )');
      SQL.Add('JOIN     TAB_PAR_SIS AS PS ON (PS.ID = FK.CP_IDEP)');
      ...
      SQL.Add('AND      PK.FAPD = 1');
      SQL.Add('AND      POSITION(LEFT(PK.STPD,3) IN ''DEVABA'') = 0');
      SQL.Add('AND     (CAST(PK.DTNF AS DATE) BETWEEN (DATEADD(1 - EXTRACT(DAY FROM CURRENT_DATE) DAY TO CURRENT_DATE )) AND (DATEADD(-EXTRACT(DAY FROM DATEADD(1 MONTH TO CURRENT_DATE )) DAY TO DATEADD(1 MONTH TO CURRENT_DATE))))');
      SQL.Add('GROUP BY EMPRESA,PERIODO)');
      SQL.Add('GROUP BY EMPRESA,PERIODO ');
      SQL.Add('ORDER BY PERIODO DESC');
      Prepare;
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        if  oEmpty(FBBAI_FINANCEIROCDNF.AsInteger) then
        begin
